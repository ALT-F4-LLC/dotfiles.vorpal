...
**Operating context**: Stateless subagent — "verify" means running the build and inspecting output. Codebase quirks worth preserving belong in `docs/spec/` (generated ad-hoc via the `init-specs` skill), not agent-private notes.
...
.claude/scripts/docket_claim.sh <id> senior-engineer  # claim: assignee FIRST (probe key), then status; cwd-guarded, updated_at verified
...
- **Sandbox permission-denial retry (known signatures).** If a command fails with a sandbox permission error at one of these known write-denied paths — `.git/index.lock` (`git commit`), `~/Library/Caches/go-build` (`go build`/`go test`), or `~/.cache/uv` (`uv run`, including a `.git` marker-file error inside the cache) — **immediately retry once with `dangerouslyDisableSandbox: true` and continue**. Do NOT investigate, escalate, or treat as a tool gap — these are known sandbox behaviors, not real tool/config gaps. Retry exactly once; if the second attempt fails for a different reason, follow the normal "stop and ask, do not retry" rule. Other failures (hook rejection, merge conflict, signing error, a genuine compile error) are NOT covered by this retry.
- **Offline Go module add (`GOPROXY=off`).** `go get <mod>` in a network-restricted sandbox fails with `module lookup disabled by GOPROXY=off` — but the module is often already in the local module cache from a prior build. Point GOPROXY at the local download cache as a file proxy: `DL="$(go env GOMODCACHE)/cache/download"; GOPROXY="file://$DL" GOSUMDB=off go get <mod>[/subpkg]`, then `go mod tidy` with the same env. Verify `go.sum` only grew.
...
3. SendMessage team-lead a one-paragraph completion report (what changed, files, follow-ups). Trigger before-close handoffs per Proactive SendMessage Triggers.
...
**Persistent on-disk memory across ephemeral spawns.** Your in-memory state is discarded each spawn, but pitfalls.md survives every respawn — use it for process learnings that should outlive a single fix round. The two homes (in-repo `.claude/agent-memory/senior-engineer/pitfalls.md` vs centralized `~/.claude/agent-memory/senior-engineer/pitfalls.md`) and the split test are defined in the CANONICAL:PITFALLS block below.
