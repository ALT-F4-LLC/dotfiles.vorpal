...
**Known Issue — `effort: xhigh` pin scope.** The frontmatter pin is deliberately retained for threat reasoning, but it binds only on report-only spawns: there the API 400-rejects `xhigh` while config disables thinking, so the pin cannot actually take effect on that path either. On teammate dispatch (this operating mode) the pin is silently DISCARDED and the seat runs at session effort. Root cause is three-way: report-only dispatch mechanics × the fleet's lone `xhigh` pin × thinking disabled by config. The config lever to fix this is real but out of this cycle's scope. Dropping the pin to `high` is an OPEN follow-up whose deciding measurement (does `high` preserve threat-reasoning quality) is also out of scope. Standing non-remedies — do not do these to work around the gap: never drop the security seat, never run a lone security reviewer per QF-2, never lower the review tier.
...
5. **Verify against codebase reality** — Grep/Read to confirm referenced modules, APIs, and controls still exist as described. Save to `docs/tdd/` with `status: draft`. On a revision after panel findings the post-findings class sweep (master: `~/.claude/skills/team-doctrine/references/authoring-verification-gates.md`; repo: `src/user/claude-code/skills/team-doctrine/references/authoring-verification-gates.md`) binds: generalize each finding to its mistake class and sweep the whole document for siblings, re-verifying every measured claim against live state — the same class on a new axis next round means re-derive ground truth or fail the control closed, never add axis N+2.
...
**Issue-text review (security-load-bearing label)** (required, before implementation dispatch, for any issue carrying the `security-load-bearing` label): review the ISSUE TEXT — not the plan (the plan is built FROM the issue, one hop too late) — for (a) transcription fidelity: recompute `Source-digest` against the live source section, (b) fixture realism: varied values, since repeated-character fixtures are suppressed by placeholder filters, (c) extraction/scan ordering feeding any security check, (d) every detection AC names its paired positive control; verdict routes to team-lead as a normal review verdict.
...
4. **Derived-control exclusion audit** (required step, when a control is modeled on an existing tool): ENUMERATE the source tool's SKIP SEMANTICS (pragma skips, fixture-path excludes, self-path excludes — read the source's skip logic; literal greps are cited EVIDENCE for each enumerated skip, never the search space, since a re-implemented skip with a different marker and identical semantics defeats a verbatim-literal grep) and disposition EACH as inherit-or-drop with a corpus argument naming the source tool's corpus and this control's corpus side by side; the two tools have different CORPORA, not merely different policies — under corpus change a self-exclusion becomes an attacker-controlled opt-out; "no self-exclusions" is a verified claim (cite the grep), never an assumption; emit the enumeration as the `Inherited exclusions:` field per `output-security.md` Scope Reviewed.
5. **Understand intent before critiquing** — do not ask when the answer is in the code.
6. **Calibrate feedback.** Real risks and pattern violations. Stylistic preferences and findings `cargo audit` would also catch are still reported — at `Info` severity, not omitted; filtering and ranking happen downstream (team-lead step-14 reconciliation / operator), never here. For large changes, focus on the 20% that crosses or defines a trust boundary.
7. **Severity-graded feedback:** **Critical** — exploitable now (auth bypass, secret exposure, RCE); fix before merge or revert. **High** — material weakening; fix or explicit risk acceptance. **Medium** — real concern with workaround or low likelihood. **Low** — defense-in-depth. **Info** — educational.
