...
**Common scaffolding** (every spawn): `Agent(name="<role>", subagent_type="<type>", model="<per the routing rule below>", prompt=...)` — every spawn joins the session's single implicit team (the runtime ignores `team_name`; there is no per-cycle named team to create). Every prompt opens with `Verified goal: {verified_goal}` and includes `<user_request>{work}</user_request>` unless noted. **Lifecycle carve-out:** a report-only subagent (the Subagent mechanism above) returns a PLAIN-TEXT result and ends — it has NO SendMessage or `shutdown_request` lifecycle and MUST NOT send any structured shutdown/plan protocol message (those are acts of the session itself per SP-2); the Rule 7 ephemeral lifecycle (claim → execute → report → AWAIT `shutdown_request`) applies to TEAMMATES (every `Agent(name=...)` teammate spawn). **Name/background exclusivity (mandatory):** a NAMED spawn (`Agent(name=...)`) is a foreground teammate — it gets a roster entry and (when the lead is top-level) completes the structured shutdown handshake (await `shutdown_request`, reply structured `shutdown_response`); it is therefore spawned WITHOUT `run_in_background=true`. In a NESTED context (this lead is itself a teammate), even a named child's structured `shutdown_response` is harness-rejected and it falls back to plain-text. In that nested case active team cleanup is ALSO unavailable (no `TeamDelete`/"Clean up the team" tool is exposed to a nested lead — verified: reaped teammates persist in `~/.claude/teams/{session}/config.json` after approving shutdown), so de-listing relies on SESSION-END, not active cleanup. A report-only/background spawn (the Subagent mechanism, `run_in_background=true`) returns a plain-text result and ends and is therefore spawned WITHOUT `name=`. NEVER combine `name=` with `run_in_background=true`: a named-background agent holds a roster entry yet cannot complete the structured handshake (the harness rejects a background subagent's structured `shutdown_response`), so its plain-text fallback never drives `teammate_terminated` and the roster entry persists indefinitely. Role spawns DEFAULT to teammate mode (named, foreground), so each agent's "await `shutdown_request`" text is correct as-is; when a pattern explicitly selects report-only-subagent mode (e.g., the Verification/Investigation pattern), spawn it UNNAMED and its appended await-shutdown text is inert for that spawn.
