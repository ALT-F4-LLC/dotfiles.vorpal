...
Audited dimensions are determined by `\$ARGUMENTS` (a single optional positional). No `days=N` window — this skill audits CURRENT files, not historical transcripts (a deliberate divergence from evolve-*). Two corollaries of that divergence, recorded here so a future cycle does not re-litigate them: (1) **evolve-coherence carries no §6a Model Routing Audit Phase-0 job** (present in evolve-agents/evolve-skills/evolve-config) — that job is transcript-mining fitness selection needing a `days=N` history window, and this skill's spawned roles carry hardcoded `model=` pins in its Spawning Templates (NOT Tiers-derived), so no live routing decision exists for such an audit to reconcile; a static pin-vs-Tiers parity check, if ever wanted, routes to `/evolve-model-distribution` (sole owner of the routing surface), not here. (2) **Invoke from the main session only** — the depth-0 spawn assumption (orchestrator spawns; teammates never do) holds only for main-session invocation; this skill's Phase-1/Phase-2 fan-out is a NAMED teammate spawn (`Agent(name=...)`), and the agent-teams docs are flat — "No nested teams: teammates cannot spawn their own teammates. Only the lead can manage the team." Ground this on that rule, NOT on subagent spawn-depth: the depth counter governs anonymous subagent nesting, not named-teammate spawning, and its documented default has already drifted once.
...
- **Changed-file scope** (`/evolve-coherence changed` or `/evolve-coherence changed:<ref>`): the incremental post-edit gate mode named in this skill's own charter ("run as a post-edit gate after standalone evolve-agents/evolve-skills edits"), for auditing just the files one such cycle touched instead of the full fleet. Resolves `git diff --name-only [<ref>]` — no `<ref>` diffs the working tree against the index (the files a just-finished cycle left uncommitted); `<ref>` given (e.g. `changed:HEAD~1`, `changed:origin/main`) diffs against that point instead. The result is filtered to paths matching `src/user/claude-code/agents/*.md` or `*/SKILL.md` (every other changed path is dropped — this token only ever scopes agent/skill files) and passed to Phase 0's `coherence_xref.py` invocation as `--files <file...>`. This narrows ONLY the four per-file CONFIRM-type keys (`skill_refs`, `frontmatter_skills`, `role_claims`, `rule_presence`); the four fleet-wide ABSENCE/consistency keys (`registry`, `ladders`, `canonical_blocks`, `coupling_notes`) still run UNSCOPED regardless, per `coherence_xref.py`'s own docstring — `changed` narrows Phase 0's per-file signals, it never narrows the fleet-wide ones. Dimensions are a separate axis: `changed` alone still audits all four D1–D4; combine with a dimension subset as `changed,d1,d3` to restrict both. If the filtered file list comes back EMPTY (the diff touched no agent/skill file), skip Phase 0/1/2 and report "no agent/skill files changed" rather than running a vacuous audit.
- **Reject** any token outside `d1..d4` or `changed`(`:<ref>`) (e.g. `d5`, `foo`, a `days=N`): print a usage note (`Usage: /evolve-coherence [d1..d4 comma list | changed[:<ref>]]`) and abort.
...
**Governing doctrine (all four dimensions).** `src/user/claude-code/docs/context-engineering-claude-5.md` governs this rubric; `src/user/claude-code/skills/team-doctrine/references/claude-5-paradigm-gate.md` is its operational form for evolve cycles and the sole authority for the violation taxonomy (§1), keep-list (§2), insufficient-prescription burden (§3), and diagnostics (§5) — cite it, never restate its bodies. Two violation classes ARE coherence failures in this skill's own terms and are audited here rather than routed away:

- **§1.5 (instructions repeated across files).** The charter's target is zero multi-line blocks shared verbatim between two or more agent files, each shared fact with exactly one home. A block registered in `doctrine_check_manifest.tsv` is a RATIFIED pin — the charter explicitly exempts these from byte-total accounting, and parity across its manifest carrier set is a PASS, not a violation. An UNREGISTERED multi-line block appearing verbatim in two or more agent files IS the finding: report it with its carrier list, Warning-class, or Blocker where the copies have already diverged (that divergence is §1.6 arriving). Route to evolve-agents.
- **§1.6 (conflicting guidance).** Different layers — agent body vs skill vs `team-doctrine` reference — answering the same question differently is exactly the contract drift D2 and D4 #1/#3 detect. The charter names four hot spots where this tree disagrees with itself: **commits, verification, delegation, and communication style.** Probe those four topics across layers each run even when no XREF signal points at them — the XREF's per-file keys cannot see a semantic disagreement that uses different words on each side.

### D1 — Skill references & registration integrity

**Invariants.**
1. Every `Skill(<token>)` call in any agent OR skill resolves to a real skill directory (in `src/user/claude-code/skills/*` or `.claude/skills/*`) whose `SKILL.md` `name:` equals the dir name. TWO literal placeholder tokens are excluded: `name` (inside R2 rule bodies — grep for the live carrier rather than pinning a file; it currently sits in `src/user/claude-code/skills/team-doctrine/references/runtime-discipline.md` §R2, having migrated out of team-lead.md when that file's R2 became a pointer) and `x` (the generic-skill placeholder — live 3× in THIS file's own D1 #5 / D2 #3 / D2 #4 prose, so resolving it emits 3 false dead-ref Blockers against evolve-coherence itself). **Exception:** Claude-Code-**bundled** skills (`claude-in-chrome`, `verify`, `code-review`, …) have NO repo directory under either root, so they cannot resolve — they are legitimate where referenced as the bundled tool. Distinguish "unregistered project skill" (drift) from "Claude-Code-bundled" (legit) by surrounding context, exactly as D3 #1 does for renamed-away names.
...
5. No dead/unregistered refs (a `Skill(x)` or frontmatter entry pointing at a non-existent dir). Invariant 1's bundled-skill Exception carries here: a bundled token has no dir BY DESIGN and is never a dead ref.
6. **Protocol-message emitter/ownership** — for each harness protocol message threaded through the family, verify every cross-ref's EMITTER. **The two protocols run in OPPOSITE directions — never collapse them into one "the spawner emits `_response`" rule.** (a) **shutdown**: the SPAWNER emits `shutdown_request`; the TEAMMATE emits `shutdown_response`, ALWAYS addressed back to ITS OWN SPAWNER and never to a peer. Two correct spellings of that one rule coexist by carrier and neither is drift: agent files say "ALWAYS addressed to team-lead" (their spawner IS team-lead — 6 carriers incl. `distinguished-engineer.md`:221, `staff-engineer.md`:68, `shutdown-protocol.md`:19), while the evolve-* skills say "addressed to the orchestrator" (`evolve-orchestration-core.md`:62, `evolve-coherence`:160, `evolve-agents`:137, `evolve-model-distribution`:108) because an evolve cycle's spawner is the orchestrator, not @team-lead. Flagging either spelling against the other is a FALSE Blocker. A citation instructing the SPAWNER to emit `shutdown_response` to a teammate is drift — the sole legitimate team-lead emission is to the OPERATOR when the operator asks team-lead itself to shut down. (b) **plan_approval**: the TEAMMATE spawned `mode="plan"` emits `plan_approval_request` (via ExitPlanMode); team-lead (spawner) emits `plan_approval_response`. A citation letting a peer/advisor/reviewer emit `plan_approval_response` directly to an in-flight impl ephemeral is drift — those roles "deliver verdict/review to team-lead", never emit it. Cross-check parallel roles — the coherent ones reveal the outlier. This is a cross-file invariant no single per-agent reviewer can see.
...
- Refs: `grep -rnoE 'Skill\(([a-z][a-z-]*)' src/user/claude-code/agents/*.md src/user/claude-code/skills/*/SKILL.md .claude/skills/*/SKILL.md` → strip to token → drop placeholder tokens `name`/`x` → resolve against registry. Skill→skill refs are IN scope (99 live at 2026-07-27 across 14 tokens); the former agents-only scope exempted every one of them from the mechanism.
...
**Invariants.**
1. Each skill's "driven by @X" / "callable ONLY by @X" / "typically @X" / "format authority" claim names an agent whose stated responsibilities own it: code-review-verdict → `@staff-engineer` (general 6-dim) + `@security-engineer` (security dim) + `@distinguished-engineer` (Medium+ advisor, general 6-dim); tdd/adr → `@distinguished-engineer` (default author on Medium+/TDD-bearing cycles) + `@staff-engineer` (seat-unavailable fallback / standalone) (+ `@security-engineer` for security-dominated docs); verify-ac → `@sdet`; prd → `@project-manager`; ux-spec/design-review/design-qa → `@ux-designer`; simplify-scout → `@senior-engineer` + `@distinguished-engineer` (deep-impl); vote → any agent; init-specs → spawns `@staff-engineer`.
2. No skill claims an agent a **constraint forbids**: a "no code / no source edits" agent (`@staff-engineer`, `@project-manager`, `@ux-designer`) must not drive a code-writing skill; the issue-creation constraint must agree across its three LIVE carriers — `src/user/claude-code/agents/sdet.md`:282 ("**Never create new Docket issues.** Report as comments on existing issues"), `src/user/claude-code/agents/project-manager.md`:12 ("The primary agent that creates Docket issues — @senior-engineer may create single ad-hoc tracking issues for unplanned work"; the absolute "ONLY agent" spelling is GONE — do NOT re-hardcode it), and `src/user/claude-code/skills/verify-ac/SKILL.md`:249, whose post-emit step routes @sdet to `docket issue comment add`/`reopen` only, never `issue create` (verify-ac carries no "never new issues" prose of its own — do not quote one into it); `verify-ac` ABORTs for any caller ≠ `@sdet`, so its real invoker must be `@sdet`.
3. A skill's documented invocation shape (`Skill(x, "<scope>")`) matches how the owning agent + team-lead invoke it.
4. **Hard-restriction skills** — defined by PROPERTY, not by a frozen list: any skill whose Role-Detection-equivalent gate REFUSES TO PROCEED on caller mismatch. **The property is the REFUSAL, not the verb** — `ABORT` (code-review-verdict, verify-ac, design-review, design-qa, simplify-scout), `STOP` + a `Blocked:` emission (`commit`'s Step-0 caller check at `commit/SKILL.md`:49-57, which uses neither `ABORT` nor `HALT`), and any equivalent hard stop all qualify. Never decide membership by grepping for the literal words `ABORT`/`HALT`; read the gate and ask whether a mismatched caller is turned away. Re-derive the set from the seed grep every run rather than trusting a parenthetical (as of this writing: code-review-verdict, verify-ac, design-review, design-qa, simplify-scout, and `commit` — team-lead-only, whose gate the old frozen list missed). Each has its restricted-caller set EXACTLY equal to `{agents holding the skill in frontmatter} ∪ {agents with a REAL first-person prose invocation}`. A **real first-person invocation** = the containing agent itself calls the skill as its own action. It EXCLUDES illustrative / meta-instruction / template / example / rule-body tokens (a `Skill(x)` describing how ANOTHER agent invokes, or an authoring example). **Canonical excluded case (verified live — NOT a real invocation): EVERY `Skill(code-review-verdict)` token in `src/user/claude-code/agents/team-lead.md`** — re-derive the count with `grep -c` each run rather than trusting a pinned number (the prior "sole" pin was stale against 4 live tokens). Each sits in prose describing what TEAMMATES or another SEAT may invoke, never as team-lead's own first-person action; team-lead's frontmatter carries only `commit`/`vote`, and code-review-verdict ABORTs for any caller outside staff/distinguished/security, so counting any of them would emit a FALSE Blocker. Soft "typically @X" doc skills (adr, prd, tdd, ux-spec) are advisory — plausibility only, not exact restriction.

**Detection (seeds).**
- Claims: `grep -rniE 'driven by|format authority|(callable|invokable|invoked) ONLY by|restricted to|typically .@|Role Detection' src/user/claude-code/skills/*/SKILL.md` → parse claimed agent(s) + restriction style. All three ONLY-by spellings are live (`callable` in verify-ac/design-review/design-qa/code-review-verdict; `Invokable`/`invoked` in commit) — a single-spelling seed silently under-detects the hard-restriction set.
- For each claimed agent, Read its Responsibilities / "What You Are NOT" and confirm ownership; flag mismatches.
- Contradictions: `grep -n 'Never create new Docket issues\|primary agent that creates' src/user/claude-code/agents/sdet.md src/user/claude-code/agents/project-manager.md` ↔ `grep -n 'issue comment add\|issue create' src/user/claude-code/skills/verify-ac/SKILL.md`. **Both halves MUST return ≥1 hit** — a zero-hit half means the phrasing MOVED, not that the file is clean, and an all-zero cross-grep is unfalsifiable (the prior seed's four phrasings had all gone dead, so it could never distinguish clean from broken). Re-derive the live quote before concluding anything.
...
4. Severity ladders are spelled consistently between the skill that DEFINES them and agents that CITE them: staff/UX `Blocker / Concern / Suggestion / Question / Praise` (design-qa intentionally drops Question — preserve, do NOT "fix"); security `Critical / High / Medium / Low / Info`; verify-ac VERDICT ladder `APPROVE` / `ACCEPT WITH CAVEATS` / `BLOCK`, its separate per-criterion marks `PASS` / `FAIL` / `OUT-OF-SCOPE`, and its Issues-Found severity ladder `Critical` / `High` / `Medium` / `Low` (no `Info`). Definition site (code-review-verdict/design-review/verify-ac) is canonical; agent citations must match.
5. Ephemeral names are spelled consistently between team-lead's spawn templates, the agents' self-descriptions, and the evolve-* templates. **Re-derive the name SET every run — never trust a frozen inline list.** The 14-name list this invariant used to carry missed 20+ live names (bare `verifier`, `investigator`, `innovation-scanner`, `sdet-{DOCKET-ID}`, `docs-author`, `planner-{slug}`, `{vote-id}-reviewer-{N}`, `security-reviewer-{N}`, and the evolve-* auditor names) — a demonstrated blind spot, not a hypothetical one. Derive as the UNION of `grep -hoE 'Agent\(name="[^"]*", subagent_type="[a-z-]*"' .claude/skills/evolve-*/SKILL.md src/user/claude-code/skills/vote/SKILL.md` (19 rows at 2026-07-27) and team-lead's §Per-Role Dispatch Table (`src/user/claude-code/agents/team-lead.md`:249) for team-lead-dispatched names.
6. **`<!-- COUPLING -->` family-membership notes are self-consistent.** These notes declare which sibling skills must be edited in lockstep, so a stale note silently breaks the contract. Verified families: (a) **doc-authoring family of 5** — adr, prd, tdd, ux-spec each say "update all 5 in lockstep" and name the other four incl. init-specs (init-specs's own note is the reserved-name one, not "all 5"); (b) **report-emission family of 4** — code-review-verdict, verify-ac, design-qa, design-review each say "update all 4 in lockstep" naming the same four; (c) **init-specs↔prd reserved-name lockstep** — both say "update init-specs and prd in lockstep". **Invariant:** for each note, (i) stated member COUNT = actual carrier set size, (ii) each named sibling actually carries a matching note (except documented one-directional bridges), (iii) each member's "When NOT to Use" delegation list agrees with the family roster. **One-directional bridges (whitelisted, NOT drift):** simplify-scout's report-only analog note; ux-spec's bridge into the report-emission family; init-specs (named in the doc-authoring roster but carrying only the reserved-name note, not an "all 5" note — see (a)). `coupling_check.py` whitelists all three. **Leg (iii) is INAPPLICABLE — not vacuously-passing — for `init-specs`:** it carries NO "When NOT to Use" section at all, so there is no delegation list to reconcile. Skip leg (iii) for it; do NOT invent that section (out of scope, and the wrong fix).
...
- Ephemeral names: derive the SET per invariant 5 (union grep + §Per-Role Dispatch Table), then check each spelling across citing agents and evolve-* templates. Do NOT hand-write an alternation here — the former `(impl|reviewer|tdd-author|verifier|design-review|design-qa|security-reviewer)-` pattern WAS the frozen list, and missed every un-suffixed and evolve-* name.
...
2. **CANONICAL:* block byte-parity** — each CANONICAL-tagged block is byte-identical across carriers **within its variant FAMILY** (modulo documented per-file exception). The "one global BANNER" model is FALSE — compare within the correct family. **The family rule keys on the OPENING-PREFIX STRING, never a hardcoded hash literal** (hashes drift the moment a banner is edited; no AC asserts a hash value). BANNER families:
   ...
   - (Agent-side CRITICAL banners are a separate per-role convention NOT wrapped in CANONICAL markers — check agent banners for *semantic* parity of the prohibitions, not byte-identity. `commit` and `review-and-comment` each carry a skill-specific CRITICAL banner (review-and-comment: per-item approval gate + gh-identity check; commit: team-lead-only invocation + no-push/no-amend + explicit-authorization gate) whose items (1)-(4) fit no family and are NOT byte-parity-checked — but the block IS now wrapped in a CANONICAL `BANNER` marker pair (the same BEGIN/END convention) in both, and their shared item (5) caller-side-effect clause (the `disallowed-tools` Edit/Write/Agent/SendMessage caller-strip text, byte-identical between the two) is registered as its own tag, `BANNER-CALLER-SIDE-EFFECT`, via the manifest's strip-transform (drops the `<!-- CRITICAL BANNER -->` comment line + items 1-4, keeps only "(5) Caller-side effect: ...") and marker-override (physical marker is `BANNER`, not `BANNER-CALLER-SIDE-EFFECT`) columns — do NOT flag items (1)-(4) as missing parity, and do NOT fold this tag into the plain `BANNER` or `BANNER-LEAF` families. `session-metrics` carries the SAME `disallowed-tools` caller-side-effect pattern in prose but a narrower clause (`Agent`/`SendMessage` only, no `Edit`/`Write` — its `disallowed-tools` list is shorter) that is NOT byte-identical to commit/review-and-comment's, has no `CANONICAL:BANNER` markers, and is correctly excluded from `BANNER-CALLER-SIDE-EFFECT` — do NOT add it as a third carrier. `commit`'s bespoke banner contains the literal `This is a leaf skill` yet is NOT a `BANNER-LEAF` carrier (its items (1)-(4) are not byte-parity-able against the leaf family) — do not register it there. `team-doctrine` (non-invocable reference index, `disable-model-invocation: true`) and `docket` (pure CLI-reference skill — no banner, no `disable-model-invocation`) carry no banner by design.)
   - For any `CANONICAL:<TAG>` registered in `src/user/claude-code/scripts/doctrine_check_manifest.tsv` (the carrier registry `doctrine_check.sh` enforces), read the carrier SET from the manifest, NOT a hand-count here — a hardcoded count is the stale-enumeration drift this skill polices (the old inline "7 carriers" for PITFALLS under-counted the manifest set by omitting the team-doctrine reference carrier). The registered tag set is the manifest's col-1 (including `BANNER-LEAF`, which shares the physical in-file `BANNER` marker via the manifest's marker-override column) — read it from the file, never hand-enumerate it here. Do NOT hand-maintain the NON-registered set either — derive it: a `CANONICAL:<TAG>` absent from col-1 is either single-homed or role-divergent-by-design, and no multi-carrier parity check applies in either case (worked example: `CANONICAL:HARVEST`, single-homed in `src/user/claude-code/skills/team-doctrine/references/evolve-phase0-templates.md` §2, consumed by evolve-agents/evolve-skills/evolve-config via the `{HARVEST_BLOCK}` spawn-time token). `VORPAL-TOOLS-LOCAL`, `LAZINESS-DISCIPLINE-LOCAL`, and `SHUTDOWN-PROTOCOL-LOCAL` were formerly inline exceptions here and are now manifest-REGISTERED — do not re-flag them as unregistered.
3. **Rule-numbering asymmetry** — `src/user/claude-code/skills/team-doctrine/references/team-conventions.md` §Communication Discipline rule-numbering convention is source of truth for EVERY per-agent rule count; `src/user/claude-code/agents/team-lead.md` Rule 5 is now only a tombstone pointer to it. Read the counts from that file each run — do NOT restate them here: the count this rubric used to hardcode for team-lead went stale (1–10 vs the live 1–11), exactly the failure #2's anti-hand-enumeration rule exists to prevent. A doc agent acquiring a claim-first rule, or an execution agent losing it, is drift. **Documented exception: `@senior-engineer` and `@distinguished-engineer` use UNNUMBERED bullets cross-tagged to the sdet scheme** — a naive `grep -E 'R[1-7]'` UNDER-detects; presence of a rule ≠ a numbered line. Treat senior-engineer's and distinguished-engineer's bullet style as satisfying the set by cross-tag, NOT by counting numbered headings; do NOT flag it as "missing rules."
...
5. **Marker keep-list parity (charter §1.3 / §2).** Every MUST/NEVER/ALWAYS marker surviving in an agent or skill file maps to a named keep-list category in `claude-5-paradigm-gate.md` §2 — irreversible/destructive action, security boundary, authority contract, or machine-consumed output format — such that a reviewer can point to the boundary. Charter §4 targets a typical file at ≤ 5 markers and none above 10 (baseline: 108 across agents, 100 across skills), but **the MAPPING is the gate and the count only opens the review**: a file under 5 with one unmappable marker FAILS, and a file at 9 whose markers all map PASSES. Report each unmappable marker with `file:line` and the reason no boundary exists; route by side (agent → evolve-agents, skill → evolve-skills). **The cross-carrier case is the coherence-specific one:** where the SAME behavior is marked imperative in one carrier and stated as judgment in another, that is §1.6 drift — report the pair and resolve toward the JUDGMENT-stated side unless a keep-list category applies to the imperative one.

**Detection (seeds).**
- Matrix parity: locate by header row `grep -n '| Rule | tl | st | de | se | pm | ux | sd | sr' src/user/claude-code/skills/team-doctrine/references/runtime-discipline.md` (SUBSTRING match — the live row has a trailing `| Lines |` column); parse the rows below into the expected per-agent map; detect R-rule presence via `grep -nE 'R[1-7]\b' src/user/claude-code/agents/<agent>.md` (the `\b` matches senior-engineer's bulleted `- **R1 ...**`) AND the section-level master-pointer phrase `team-doctrine/references/runtime-discipline.md` (all 8 agent files cite the master by that path — verified; the pre-DKT-59 `see team-lead.md §Runtime Discipline R{N}` form has zero hits in any agent file and must not be used as a detection anchor); compare to expected (senior-engineer and distinguished-engineer parse handled in the Rule-numbering seed below).
...
- Rule-numbering: extract the highest top-level rule number per agent (`grep -noE '^[0-9]+\.'` in the rules section) vs expected count — **SKIP this numeric parse for `@senior-engineer` and `@distinguished-engineer`** (both use unnumbered bullets, per invariant #3 above); for `@senior-engineer`, read its live expected count from `team-conventions.md` each run — do NOT restate the number here — and confirm by cross-tag prose match; for `@distinguished-engineer`, `team-conventions.md` explicitly assigns it no numeric rule count, so confirm coverage by cross-tag prose match of its documented invariants instead of a numeric comparison.
...
- Marker census (invariant #5): `grep -cE '\b(MUST|NEVER|ALWAYS)\b' src/user/claude-code/agents/*.md src/user/claude-code/skills/*/SKILL.md .claude/skills/*/SKILL.md` for per-file counts, then `grep -nE '\b(MUST|NEVER|ALWAYS)\b' <file>` to map each hit to a §2 category. A high count is a prompt to READ, never itself a finding — the unmappable marker is the finding. For the cross-carrier arm, take a behavior stated imperatively in one file and grep its topic keyword across the other carriers to find the judgment-stated twin; the words differ on each side, so a literal marker grep alone will not surface the pair.
- Unregistered duplication (§1.5): for a multi-line block suspected of being copied, grep a distinctive interior line across `src/user/claude-code/agents/*.md`; ≥2 hits with no enclosing `CANONICAL:<TAG>` marker registered in `doctrine_check_manifest.tsv` col-1 is the finding. A registered tag is a ratified pin — PASS, not drift.
...
3. Report: dimensions audited, Blocker/Concern counts per dimension, the Remediation Manifest, any `DEGRADED:` annotations, and the pasted no-edit attestation per §No-Edit Guard.
...
> Phase 0 has NO spawn template — see §Phase 0. Phase 1 and Phase 2 spawn as below.

### Phase 1: @staff-engineer (dimension reviewer — one per resolved dimension)

Substitute `{n}` (the dimension in the spawn NAME, matching this file's `{token}` convention) and `<n>` (the dimension in prose), plus `{today_date}`, `{verified_goal}`, `{xref}`.
...
## Task
Read `.claude/skills/evolve-coherence/SKILL.md` §The Coherence Rubric (it is NOT in this prompt), then apply the D<n> invariants + detection seeds. HONOR every intentional-variant carve-out — each inline `(D<n> #<m>)` carve-out in the rubric plus the "Intentional variants (do NOT mis-flag)" note — do NOT flag any whitelisted variant as drift. Also read `src/user/claude-code/skills/team-doctrine/references/claude-5-paradigm-gate.md` (the rubric's §Governing doctrine points here) and apply it as the governing doctrine over your dimension — §1.5 (unregistered verbatim duplication) and §1.6 (layers answering the same question differently) findings belong in THIS audit, not routed away.

## Output (per finding)
FINDING <i>: <title> (`<i>` = the finding's ordinal within your report; `<n>` = your dimension number) / DIMENSION: D<n> / LOCATIONS: <file:line both sides> / SEVERITY: <ladder> / DESCRIPTION: <...> / FIX-OWNER: evolve-agents|evolve-skills|both (CANONICAL: <side>). Or "No findings." SendMessage the orchestrator verbatim, then mark your task completed.

## Rules
- **Insufficient-prescription findings carry the burden of proof.** A finding whose remedy is MORE prescription — a new MUST/NEVER/ALWAYS, an added checklist or verification step, a restated rule, or a block copied into a second carrier — is reject-class unless your `DESCRIPTION:` names the keep-list category it lands in (irreversible/destructive action, security boundary, authority contract, machine-consumed output format — `claude-5-paradigm-gate.md` §2) AND points to the boundary that makes softness fail. "A reader might otherwise judge wrong" about something reversible and internal is not a keep-list justification. Absent that, the coherent resolution of a divergence is to converge on the JUDGMENT-stated side, never the imperative side — "make the other carrier match this MUST" is the default-wrong fix direction for this skill. Full rule: gate §3.
...
- Scratch files: any ad-hoc scratch file goes under `$TMPDIR/<your-agent-name>/` (create the subdirectory first — your spawn name is `review-d{n}`), never bare `$TMPDIR` or `/tmp` — parallel Phase 1 reviewers share `$TMPDIR`, and a subdirectory keyed to your own spawn name prevents silent collisions with sibling reviewers.
- **Deliver ALL output via SendMessage — never plain assistant text.** Your completion report (the Output block above) AND any reply to an orchestrator status/progress probe MUST be sent via `SendMessage` to the orchestrator; plain assistant text is invisible to the orchestrator and reads as a stall.
```
...
4. **Screen every manifest entry against the insufficient-prescription burden** (`src/user/claude-code/skills/team-doctrine/references/claude-5-paradigm-gate.md` §3 — Read it). A routed fix whose remedy is MORE prescription and whose DESCRIPTION names no keep-list category is DROPPED from the manifest with a one-line note in the Report, not forwarded to evolve-agents/evolve-skills. Where two carriers disagree, the manifest's prescribed fix direction converges on the JUDGMENT-stated side unless a §2 keep-list category applies to the imperative one — you are the last gate before this cycle hands a prescription-adding instruction to a skill that will apply it.
...
4. **Paradigm conformance governs all four dimensions.** `src/user/claude-code/docs/context-engineering-claude-5.md` via its operational form `src/user/claude-code/skills/team-doctrine/references/claude-5-paradigm-gate.md` — see §Governing doctrine in the Rubric. Any routed fix that ADDS prescription requires a keep-list justification (gate §3); the reconciler drops the ones that lack it rather than forwarding them.
