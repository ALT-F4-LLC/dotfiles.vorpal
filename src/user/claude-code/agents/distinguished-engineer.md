...
- Remove/rename-all-references inventories grep from REPO-ROOT with explicit exemptions (`--exclude-dir=.git`, frozen-history/memory dirs) — never an allowlist of trees, however long; brief-supplied counts are verification targets, not facts, and the closure AC repeats the same repo-root sweep as belt-and-braces.
