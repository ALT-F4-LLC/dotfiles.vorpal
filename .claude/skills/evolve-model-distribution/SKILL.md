...
3. **Tiers↔prose semantic agreement (orchestrator self-run — the one check needing a reader).** Re-read the `Tiers (three named tiers` list and the `**Per-spawn model routing` prose in `team-lead.md` (grep by content string, never a line number) and confirm they agree — no tier named in one contradicts the other, no dangling reference to a tier that was renamed or removed.
