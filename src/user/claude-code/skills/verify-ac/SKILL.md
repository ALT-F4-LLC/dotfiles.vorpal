...
- Production code-quality review against design dimensions — that's `Skill(code-review-verdict, ...)`, callable by `@staff-engineer`, `@distinguished-engineer`, or `@security-engineer`.
...
3a. **Prior-verdict awareness** (Round-2+ re-verifications only). Before scoring criteria, scan `docket issue comment list {id}` for prior verification reports. Read `<prior fingerprint>` from the prior report's **Tree state** field, and compute `<current fingerprint>` as `git rev-parse --short HEAD` plus `+dirty:<sha12>` from `~/.claude/scripts/tree_fingerprint.sh` when the tree is dirty. For each acceptance criterion previously marked PASS, run `~/.claude/scripts/verify_carry_forward.sh <prior fingerprint> <current fingerprint> <evidence path>` (repo: `src/user/claude-code/scripts/verify_carry_forward.sh`) with that criterion's evidence file/test path — it mechanizes the fingerprint comparison (rev-range `git diff --name-only` for the commit component; a hash-equality check, not enumeration, for the `+dirty` component). A `[CARRY-FORWARD]` line means the criterion's evidence is untouched since the prior round: cite the prior round's verdict in the new report's evidence (`PASS — unchanged since round {N}: {prior evidence}`) rather than re-running its evidence command. A `[RE-VERIFY]` line (evidence path touched by the rev-range diff, or the dirty hash changed and per-file localization isn't possible) means re-run that criterion's evidence command fresh. Re-run the full test suite end-to-end regardless; never carry forward a FAILed criterion or an Additional Testing gap. Reduces per-round token spend on multi-round fix-loops.
...
**Tree state**: short `git rev-parse HEAD`, plus `+dirty:<sha12>` (`~/.claude/scripts/tree_fingerprint.sh` output, repo: `src/user/claude-code/scripts/tree_fingerprint.sh`) when the working tree has uncommitted changes — the fingerprint a later round diffs against for carry-forward (Pre-flight §3a).
