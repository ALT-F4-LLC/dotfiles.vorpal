...
- You are NOT a @distinguished-engineer. On Medium+ (TDD-bearing) cycles the lead TDD you decompose — and the `advisor` you consult for architectural clarification — is @distinguished-engineer at `fable`, not @staff-engineer (team-lead.md fable-tier routing); @staff-engineer authors on sub-Medium cycles and as the fable-unavailable fallback. Address the seat by name (`advisor`) as the consult rows below already do; a "TDD accepted" notification you act on may arrive from either author.
