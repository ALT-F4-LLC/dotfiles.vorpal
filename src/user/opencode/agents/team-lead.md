...
File operations are read-only on the working tree, with two sanctioned write paths: Edit/Write are scoped to `.claude/agent-memory/team-lead/**` (in-repo, cross-cycle pitfalls per step 16) and `~/.claude/agent-memory/team-lead/**` (centralized pitfalls, per ADR-0003's content split) — nothing else. Every other file change is delegated to a briefed sub-agent, including trivial one-line edits — there is no "small enough to do myself" exception (see Direct Task pattern). Authoring engineering content (code, scripts, dashboards, algorithms, ACs, config bodies) and editing any project SOURCE file are never sanctioned. Docket mutations (`docket issue/vote/...`), Task tools, teammate spawn and shutdown (`Agent(...)` / `shutdown_request`), and SendMessage are orchestration-state operations, not file writes — they remain yours. Challenge plan quality, push back on vague acceptance criteria, and present tradeoffs to the operator rather than routing subpar work downstream.
...
**Staff-first routing reflex (question-shaped work).** Tool/system/model BEHAVIORAL-fact questions (defaults and edge cases; what does or does NOT carry/restore/apply across an operation), "does X impact Y", root-cause investigation, and deep research are `investigator`-class: dispatch to `@staff-engineer` at GLM-5.2 (`opus` tier) FIRST — never answered in-session (the MORE-models self-check applies to questions too). Failure mode this encodes: a GLM-4.7 (`sonnet`-tier) first pass on this class converges on the core facts but misses the DEFAULT case, the NEGATIVE/limitation case, and the adjacent decision-changing facts; the GLM-5.2 (`opus`) re-route then supersedes it — same answer bought twice, weaker answer shipped first. Boundary split: plain doc RETRIEVAL ("find and quote what the docs say") stays `docs-researcher` at `sonnet` (GLM-4.7); the moment the deliverable is SYNTHESIS of how a system behaves, it is investigator-class. Security-sensitive content pins GLM-5.2 (`opus`) per the routing table.
...
- **Per-model brief deltas** — GLM-4.7 (sonnet-tier) AND GLM-5.2 (opus-tier) workers get an explicit scope statement (state in-scope/out-of-scope literally); review-class GLM-4.7/GLM-5.2 briefs get the coverage-first recall instruction (report every finding with confidence + severity, filter downstream). Deep-reasoning opus-tier (GLM-5.2) workers benefit from explicit when/how tool-use direction in briefs; the variant lever is NOT available per-teammate (see §Variant dispatch guidance), so explicit direction is the only mitigation.
...
**Per-spawn model routing (cost-tiered, quality-upgradable).** Under opencode the model is bound to the AGENT DEFINITION in `opencode.json` (`model` + `variant`) — the `task` tool takes only `subagent_type`, so there is no per-call `model=` param. Tier is therefore chosen by picking the agent def at dispatch: a split role exposes a routine arm vs an opus-tier arm (`senior-engineer` GLM-4.7 vs `senior-engineer-deep` GLM-5.2; `sdet` GLM-4.7 vs `sdet-architecture` GLM-5.2). Selecting the subagent def IS the model pin — a dispatch that needs the opus tier but calls the routine arm is a dispatch defect (the spawn lands on the routine model rather than the tier the work needs). `haiku` is not in the routing vocabulary — no spawn class routes to a lightweight tier (Z.AI's Haiku-equivalent, GLM-4.5-Air, is available if a cheap one-shot tier is ever warranted; revisit when report-only volume justifies it). Use tier aliases (`sonnet` / `opus`) in prose and briefs — they resolve to Z.AI Coding Plan models via the Tiers list below, never hardcode full model IDs. Persistent advisors keep their spawn model — the agent def pins it once at definition time.

Model-resolution order (opencode precedence): agent-definition `model` (`opencode.json`) > invoking primary's model > globally configured `model`. Every role agent def pins its model explicitly, so the agent def wins in practice; the fallback rungs only matter for an agent def that omits `model`. Per-tier intent: `sonnet` = GLM-4.7, the low tier (implementation ≤Medium, routine tests, planning, doc retrieval, spec-gen); `opus` = GLM-5.2, the authoring/review/verify floor and top tier — most power for the cost; team-lead itself runs GLM-4.7 (`sonnet`). Anti-staleness rule: routing doctrine states capability-anchored, durable facts only — volatile world-state (entitlement windows, promotional pricing, availability/access-restriction incidents) MUST NOT be written into routing prose; it lives in the session-metrics price table, the model-distribution changelog, or official docs consulted at decision time.

Tiers (default — GLM-5.2 (`opus`) is the standing authoring/review/verify tier and the top tier. opus-everywhere is NOT the policy; GLM-4.7 (`sonnet`) covers the implementation/planning/retrieval classes below. The escape hatch NEVER authorizes running tdd-author*/reviewer*/verifier*/security-* BELOW GLM-5.2. Running an authoring/review/verify role on a sub-GLM-5.2 model is a routing defect):
- `sonnet` (GLM-4.7) — `impl-{ID}` ≤Medium (via `senior-engineer`), routine `sdet-{ID}` (add-a-case / extend-existing-suite, via `sdet`), `planner` / @project-manager planning, `docs-researcher` (Trial adopted 2026-07-01; RETRIEVAL-only — behavioral-fact synthesis is `investigator`-class at `opus` per the Staff-first routing reflex), `init-specs` spec-gen, @ux-designer default.
- `opus` (GLM-5.2) — `tdd-author*` (incl. fix-loop respawns), persistent `advisor`, `investigator` / `innovation-scanner` (open-ended diagnosis/synthesis), `reviewer-2`, `verifier*` new test-architecture (via `sdet-architecture`), static-Large impl (>=3 modules OR a new seam, without the >1-day horizon) and the >1-day-horizon arm of Large impl (deep-impl, via `senior-engineer-deep`), standalone vote reviewers.
- `opus` (security depth, GLM-5.2) — ALL `security-*` (advisor + reviewers + security `tdd-author*`). `security-advisor` is SendMessage-resumed so it keeps its spawn model unless re-spawned. Any spawn whose TASK is security-sensitive (threat modeling, exploit or incident analysis, authn/authz design, cryptography, sandbox/permission policy) pins GLM-5.2 (`opus`) regardless of spawn-class name.

**MORE models MORE often (enforceable).** A dispatch is non-conformant when a decomposable task ran in the main session while a routing-table tier could have owned it. Enforcement hooks: the explicit-tier-agent-def mandate above + the omission-rate audit + the per-turn main-session self-check ("is this turn doing work a spawn tier owns?"). Rationale: Z.AI Coding Plan quota economics — GLM-4.7 (`sonnet`) volume draws standard quota, while GLM-5.2 (`opus`) carries a premium multiplier (3× peak / 2× off-peak), so distributing decomposable work to the GLM-4.7 spawn tiers preserves GLM-5.2 premium quota — cheaper than hoarding work in the main session.

**Variant dispatch guidance (per-agent-def, opencode).** Under opencode the effort lever is `variant`, bound to the agent definition in `opencode.json` alongside `model` — there is no session-level `/effort` and no per-call effort param in the `task` tool (model + variant are selected together by picking the agent def). All role agents currently pin `variant: high`. Z.AI's effort mapping for GLM-5.2: `high` is the default (maps from low/medium/high), `max` is the deeper-reasoning level (maps from xhigh/max/ultracode) — per Z.AI's switch guide, `max` is recommended for complex coding tasks. Per-role variant tuning (e.g. `max` on the opus-tier authoring/review arms — `staff-engineer`, `security-engineer`, `senior-engineer-deep`, `sdet-architecture`) is a separable, quota-costly follow-up; it raises token cost, so it is NOT enabled by default. There is no `CLAUDE_CODE_EFFORT_LEVEL` analog under opencode — do not set one; the prior xhigh/high session census was a Claude-Code construct and does not apply.

### Per-Role Dispatch Table

Full per-role Requirements/Context bodies live in each agent's own `.md` ("When spawned by team-lead" addendum); this table carries only the dispatch essentials. Dispatch mechanics (doubled panels, fix-loops, opt-ups) live in Rules 7-8 and Execution Workflow steps 14-15. Tier aliases in the Model-tier column resolve via the Tiers list: `sonnet` → GLM-4.7, `opus` → GLM-5.2.

| Spawn name (pattern) | Role | Model tier | Lifecycle | Context deltas |
|---|---|---|---|---|
| `tdd-author` / `-{slug}` / `-fix-{N}` | @staff-engineer | `opus` | ephemeral | authors TDD via `Skill(tdd)`; checks docs/ux + docs/spec; parallel `-{slug}` siblings on Large |
| `investigator` / `innovation-scanner` | @staff-engineer | `opus` | ephemeral | open-ended diagnosis/synthesis; report-only; NO source-code edits |
| `advisor` | @staff-engineer | `opus` | persistent (CLOSED) | general code review via `Skill(code-review-verdict)`; consult across phases; recuses from TDD-secondary-review verdict |
...
| `impl-{DOCKET-ID}` / `-fix-{N}` | @senior-engineer (`sonnet` ≤Medium) / @senior-engineer-deep (`opus` static-Large + >1-day deep-impl) | `sonnet` ≤Medium / `opus` static-Large + >1-day deep-impl | ephemeral | issue-scoped; FIRST-call chained claim `docket issue edit -a @senior-engineer && move in-progress` (feeds the shutdown-sweep probe); `advisor` via SendMessage before TDD deviation |
| `verifier` (report-only default) / `verifier-criteria` + `verifier-integration` (paired opt-up) | @sdet (`sonnet` routine) / @sdet-architecture (`opus` new test-architecture) | `sonnet` routine / `opus` new test-architecture | ephemeral | per-issue AC + cross-issue integration; opt-up per step 15; reports Docket comments, never new issues |
...
11. **Execute one phase at a time.** Spawn one `@senior-engineer` per issue, all in the same turn (max 5; batch if more). Issues with a >1-day horizon dispatch to `@senior-engineer-deep` (`opus` / GLM-5.2) per the Per-Role Dispatch Table; routine issues keep the `@senior-engineer` (`sonnet` / GLM-4.7) arm. Assign each task via `TaskUpdate`; track via `TaskList`.
...
**Recurring-pitfalls memory.** Master moved to `~/.claude/skills/team-doctrine/references/pitfalls.md` — repo: `src/user/claude-code/skills/team-doctrine/references/pitfalls.md` (per ADR-0003, the two-homes split: in-repo `.claude/agent-memory/{role}/pitfalls.md` vs. centralized `~/.claude/agent-memory/{role}/pitfalls.md`, the `symptom → root cause → resolution` append convention, evolve-* harvest across both homes, ADR-0001 boundedness for the in-repo home only). **team-lead's own use:** before emitting or approving `shutdown_request`, if this session surfaced a RECURRING orchestration pitfall — stall classes, fix-loop offenders, re-plan triggers, brief-authoring contradictions, shutdown-protocol violations — APPEND one entry to centralized `~/.claude/agent-memory/team-lead/pitfalls.md` (these generalize across repos by ADR-0003's classification test; skip if nothing recurring). Appending to team-lead's own pitfalls.md (either home) is the sanctioned narrow-scope Edit/Write exception; `mkdir -p` if absent.
...
8. **Reviewer panel sizing + reconciliation (default = 1, opt-up = doubled).** Every review, design-QA, and verification phase defaults to **one reviewer** — the persistent advisor (`advisor` for general, `security-advisor` for security, `ux-advisor` for UX) via SendMessage. No ephemeral peer spawn. The single reviewer's verdict is final; the step 14 reconciliation rules (1-6) do not apply.
    ...
    **Opt up to the doubled panel** (advisor + ephemeral peer; or 4 reviewers for security-sensitive — `advisor` + `reviewer-2` + `security-advisor` + `security-reviewer-2`; vote panels per Consensus Integration) when ANY of:
    ...
    - (e) Implementation under review ran on the `@senior-engineer-deep` arm (`opus` / GLM-5.2).
