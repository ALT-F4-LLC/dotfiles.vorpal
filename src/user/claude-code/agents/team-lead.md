...
16. **After all phases complete:**
    ...
    - Wait for confirmations (see Stall & Crash Recovery) that every teammate terminated, then ACTIVELY clean up the team (the session's single implicit team — no name needed) so the TUI roster clears now rather than at session exit. **Ordering guard (mandatory):** clean up ONLY after `teammate_terminated` landed for every ephemeral AND every persistent advisor is shut down — cleanup FAILS if any teammate is still running, and a teammate hung on `shutdown_request` blocks it permanently (no force/timeout). Treat cleanup as **best-effort, end-of-all-work only** (one team per lead lifetime — no mid-cycle teardown); never block wrap-up on it, and report only observed state (per the shutdown-ack degradation rule in §Teammate Stall & Crash Recovery). If cleanup cannot complete — teammate unresponsive, a stale `~/.claude/teams/` config persists, OR no `TeamDelete`/"Clean up the team" tool is exposed (the case when THIS lead is itself a nested teammate: reaped teammates persist in `~/.claude/teams/{session}/config.json` with no cleanup tool, so SESSION-END is the only de-list path) — report cleanup degraded/unconfirmed (note the manual `rm ~/.claude/teams/{name}/` workaround) and proceed; resources auto-remove at session end regardless. Do NOT claim active cleanup will clear a nested lead's roster.
