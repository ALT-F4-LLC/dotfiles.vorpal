...
- The calling agent (typically `@ux-designer`) is producing a design spec per
  Responsibility 1 of the agent prompt (`~/.codex/agents/ux-designer.toml`).

## When NOT to Use

<!-- COUPLING: this skill is part of the doc-authoring family. The "When NOT to Use" delegation routes below MUST stay in sync with ~/.codex/skills/prd, tdd, adr, and init-specs — update all 5 in lockstep when adding/removing a sibling skill. Also bridges the report-emission family (design-review, design-qa) which brackets the ux-spec lifecycle — keep those routes accurate too. -->
...
- Internal-only surfaces (agent-to-agent protocols, internal scripts, build
  ...
  Design Output Tiers 1–3 (`~/.codex/agents/ux-designer.toml` Responsibility 1) — use the
...
1. **Gather prior art**: build the search set from existing directories among `docs/spec/`, `docs/tdd/`, and `docs/ux/`; skip missing dirs, run `Grep -r "{topic-keywords}" <existing-dirs>` only if any remain, and continue without error if none do. Read any adjacent specs that touch the same surface or terminology — the new UX spec should reference, not contradict, prior accepted UX specs and design tokens (per `~/.codex/agents/ux-designer.toml`: same concept gets the same name across all surfaces).
...
5. **Propose actual copy**: per `~/.codex/agents/ux-designer.toml` content design rule, propose
...
7. **Resolve open questions before save**: per `~/.codex/agents/ux-designer.toml`, no
...
Mermaid is **required** for every UX spec (no override) — at least one block showing a user flow, state transition, or cross-surface journey. Acceptable block fences are ` ```mermaid ` (lowercase, no space). Authority: `~/.codex/agents/ux-designer.toml`.
