...
**Detection + re-spawn.** Source: §Crash & Stall Recovery in `src/user/claude-code/skills/team-doctrine/references/evolve-orchestration-core.md` — binds as written there (detection signals (a)-(d), nudge-before-re-spawn, the `-r2` re-spawn contract); cite it, never restate it. Read it once at Phase-0 spawn time (same Read-once convention as `evolve-phase0-templates.md`); if the file or a named section is missing, ABORT the cycle loudly.
- **Second failure**: §Second-Failure Recovery in `src/user/claude-code/skills/team-doctrine/references/evolve-orchestration-core.md` is the sole authority — cite it, never restate it.
