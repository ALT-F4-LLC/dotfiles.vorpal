...
After the per-agent blocks, append the verbatim **CROSS-PROJECT PITFALLS MANIFEST** — the full sorted `find` output grouped by repo root (the delete-candidate set the orchestrator consumes in Wrap-up). If the scan found nothing, write `CROSS-PROJECT PITFALLS MANIFEST: none`.
