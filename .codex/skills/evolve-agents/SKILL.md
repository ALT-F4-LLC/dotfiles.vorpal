...
2. **Outcome signals per model** — for each agent/model pair observed, correlate with:
   - Stall signals: grep `wait_agent` timeout/error markers near the agent name or agent ID; count distinct events by agent ID plus session identifier.
   - Fix-loop respawns: count local phase-ledger entries for the same logical task with `attempt` greater than 1; if the ledger is absent, report `none`.
...
4. **Codex memory** — `grep -lri 'model\|routing\|gpt\|codex' "$CODEX_HOME/memories" .codex/agent-memory/ 2>/dev/null` for any durable routing lessons already recorded.
