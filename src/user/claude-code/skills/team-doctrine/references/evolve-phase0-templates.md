...
1. **Transcripts**: prefer `bash src/user/claude-code/scripts/tool_call_frequency.sh --since {history_days} --tool <name>|--pattern <regex>` (at least one filter required; NB `--since` here takes the DAY-COUNT `{history_days}` — unlike `evolve_signals.py --since {history_cutoff_iso}`, which takes the ISO cutoff) — it wraps the recent_transcripts.sh → jq tool-call extraction → per-command distinct-session counting pipeline this step previously hand-rolled, and de-dupes per session by construction. Fall back to `bash src/user/claude-code/scripts/recent_transcripts.sh {history_days} -0` + per-file grep only for shapes the script's filters cannot express (e.g. Read-path recurrence). Identify same/near-identical patterns recurring across DISTINCT `sessionId`s — mirror historical-auditor's "De-dupe before counting" discipline (never count replicated lines within one session).
