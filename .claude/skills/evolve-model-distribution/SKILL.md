...
1. **Recover the pinned literal from its owning file** — `grep -rn 'Agent(name="<role>"' .claude/skills/ src/user/claude-code/skills/`. BOTH roots are mandatory: the evolve-* Phase-0 auditor roles pin their literal in the SHARED `src/user/claude-code/skills/team-doctrine/references/evolve-phase0-templates.md`, and non-evolve skills spawn too (`vote`, `init-specs`) — a `.claude/skills/`-only glob silently under-scopes and re-creates the same silent drop. When a role hits BOTH roots the shared template is the SOURCE and an evolve-* SKILL.md `Source:` line restating it is a summary; report a disagreement between them, never silently pick one. On a miss, retry the stem — spawn names instantiate placeholder templates (`review-<name>` → `review-evolve-skills`, `review-d<n>` → `review-d3`) and carry `-r2` / `-fix-{N}` / `-retry` suffixes. Still no hit → bare-name grep (`grep -rn '<role>' .claude/skills/ src/user/claude-code/skills/`) before declaring unrecoverable: some owners declare a spawn in roster-table/prose only with no `Agent(name=` literal (e.g. `history-compactor`) — a prose hit identifies the owner; report its `model=` as unpinned.
...
4. **Unrecoverable** (no `Agent(name=` hit in either root) — report the role + measured tier as unattributed; never guess an owner.
