...
- **Diff-scoped secret-pattern scan** (new secrets about to land in the reviewed diff — distinct from the `.env`-handling audit above) → consume the pre-computed `.claude/scripts/secret_scan.sh <diff-scope>` result folded into team-lead.md Rule 8(d)'s shared brief (doubled/security-track panels) as-is: added-lines-only, redaction-only findings, self-excluding its own path and `.claude/scripts/test/`, always report-only (exit 0). It has NO disk cache (unlike (c)'s `audit_snapshot.sh`) — on single-reviewer (default) reviews, or to re-check after further edits land, re-run `.claude/scripts/secret_scan.sh <diff-scope>` directly.
