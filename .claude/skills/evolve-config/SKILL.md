...
2. **Behavioral** — Does the setting change the deployed `settings.json` (or a script's behavior)? Reject: settings that serialize identically to the current output (no-op `with_*` calls). `src/user/claude-code/scripts/config_render_diff.sh '<render-cmd>'` mechanizes this by byte-diffing the rendered config at HEAD vs. worktree (exit 0=differs/PASS, 1=identical/FAIL) — but no `claude_code.rs` render target prints the full config yet (see the script header's DKT-309 gating), so the serde-attribute read (Phase 1 step 3) stays primary until it lands.
...
<!-- CANONICAL:CRASH-STALL-CORE:BEGIN -->
...
<!-- CANONICAL:CRASH-STALL-CORE:END -->
