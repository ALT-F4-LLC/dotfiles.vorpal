...
- Snapshots the `agents/` and `skills/` source directories into artifacts via `FileSource`.
...
### 4. Agent-team & skills payload (`agents/`, `skills/`, `.claude/skills/`)

A non-code payload consumed by Claude Code at runtime, not by the Rust build:

- `agents/*.md` — six agent personas (team-lead, staff-engineer, senior-engineer, project-manager, sdet, ux-designer).
- `skills/*` — project skills (adr, code-review-verdict, prd, tdd, ux-spec, verify-ac, vote, design-*, simplify-scout, init-specs).
...
graph TD
    subgraph repo["This repository"]
        ...
        PAYLOAD["agents/ + skills/ + .claude/skills/<br/>shell scripts"]
    end
...
sequenceDiagram
    ...
    Bin->>SDK: FileSource(agents/), FileSource(skills/)
...
- **Payload boundary:** `agents/` and `skills/` are data, not code. They cross into the system only via `FileSource` snapshots and are otherwise untyped and unvalidated by the Rust build.
...
- **Untyped agent/skill payload.** The `agents/` and `skills/` markdown payload is shipped verbatim with no schema validation in the Rust build. Errors in agent/skill definitions are only caught at Claude Code runtime, not at build time.
