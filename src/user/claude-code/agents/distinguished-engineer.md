...
**Docket CLI (this role).** Invoke `Skill(docket)` for the full CLI reference. Most-used: `~/.claude/scripts/docket_claim.sh <id> distinguished-engineer` (claim: assignee-first-then-status in one call, first tool call) / `docket vote list --json` (post-error retry gate — confirm before re-creating) / `docket vote create` (via `vote_delegate.sh`, see Consensus Voting). See `Skill(docket)` for the full command table, vote/doc subcommands, and senior-engineer.md's adopted `docket issue show|comment|log|graph` flow (deep-impl adopts senior's execution contract by reference).
