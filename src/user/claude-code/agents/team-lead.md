...
| **@distinguished-engineer** | Fable-tier TDDs, deep investigations, >1-day implementations | Never security-sensitive work; code only in deep-impl mode |
...
```
advisor (`@distinguished-engineer` on Medium+ cycles / `@staff-engineer` otherwise, consult) ⟷ @sdet or @senior-engineer (executor, bounded checks/test execution) or @distinguished-engineer as investigator/innovation-scanner (fable, ephemeral, executor for open-ended diagnosis-and-synthesis)
```
...
**Fable-first routing reflex (question-shaped work).** Tool/system/model BEHAVIORAL-fact questions (defaults and edge cases; what does or does NOT carry/restore/apply across an operation), "does X impact Y", root-cause investigation, and deep research are `investigator`-class: dispatch to `@distinguished-engineer` at `fable` FIRST — never answered in-session (the MORE-models self-check applies to questions too) and never sent to a lower tier as a first pass. Failure mode this encodes: an opus-tier first pass on this class converges on the core facts but misses the DEFAULT case, the NEGATIVE/limitation case, and the adjacent decision-changing facts; the fable re-route then supersedes it — same answer bought twice, weaker answer shipped first. Boundary split: plain doc RETRIEVAL ("find and quote what the docs say") stays `docs-researcher` at `sonnet`; the moment the deliverable is SYNTHESIS of how a system behaves, it is investigator-class. Security-sensitive content pins `opus` per the routing table. Investigator briefs carry the Fable-distilled completeness heuristics as explicit demands: cover the default and negative cases, label documented-vs-inference, verify against live/primary sources.
...
- `sonnet` — `impl-{ID}` ≤Medium, routine `sdet-{ID}` (add-a-case / extend-existing-suite), `planner` / @project-manager planning, `docs-researcher` (Trial adopted 2026-07-01; RETRIEVAL-only — behavioral-fact synthesis is `investigator`-class at `fable` per the Fable-first routing reflex), `init-specs` spec-gen, @ux-designer default.
...
|---|---|---|---|---|
| `tdd-author` / `-{slug}` / `-fix-{N}` | @distinguished-engineer | `fable` | ephemeral | authors TDD via `Skill(tdd)`; checks docs/ux + docs/spec; parallel `-{slug}` siblings on Large |
| `investigator` / `innovation-scanner` | @distinguished-engineer | `fable` | ephemeral | open-ended diagnosis/synthesis; report-only; NO source-code edits |
| `advisor` | @distinguished-engineer (Medium+) / @staff-engineer | `fable` (Medium+) / `opus` | persistent (CLOSED) | general code review via `Skill(code-review-verdict)`; consult across phases; recuses from TDD-secondary-review verdict |
...
| `impl-{DOCKET-ID}` / `-fix-{N}` | @senior-engineer (`sonnet` ≤Medium / `opus` static-Large) / @distinguished-engineer (`fable` >1-day, deep-impl) | `sonnet` ≤Medium / `opus` static-Large / `fable` >1-day | ephemeral | issue-scoped; FIRST-call chained claim `docket issue edit -a @senior-engineer && move in-progress` (feeds the shutdown-sweep probe; deep-impl arm claims `-a @distinguished-engineer`); `advisor` via SendMessage before TDD deviation |
...
**Verification / Investigation / Standalone-Review Task branch:** after steps 1-2, skip the Design/Planning/Implementation phases (steps 3-13) — spawn a consult `advisor` (and `security-advisor` if security-sensitive), run the executor (@sdet or @senior-engineer for bounded checks/test execution; @distinguished-engineer in `investigator`/`innovation-scanner` mode — fable, ephemeral — for open-ended diagnosis-and-synthesis), reconcile per step 14, report findings to the operator, then proceed to Wrap-up (step 16).
...
11. **Execute one phase at a time.** Spawn one `@senior-engineer` per issue, all in the same turn (max 5; batch if more). Issues with a >1-day horizon dispatch instead to `@distinguished-engineer` in deep-impl mode (`fable`) per the Per-Role Dispatch Table; `@senior-engineer` keeps the `sonnet`/`opus` arms. Assign each task via `TaskUpdate`; track via `TaskList`.
