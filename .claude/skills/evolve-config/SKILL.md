...
A trait here is one config setting (a `with_*` call, an env var, a permission/sandbox rule, a hook wiring) or one script behavior. Selection acts on settings that demonstrably reduce a failure class (a permission prompt that recurs, a sandbox rule that blocks legitimate work, a missing hook reminder) or that are obsolete/superseded by a platform change.
...
**Target selection is structural, NOT auditor-derived (MC2).** The no-signal trait set is materialized by the orchestrator from file STRUCTURE, never from the Phase 0 auditor's narrative output — call `src/user/claude_code/scripts/drift_target.sh .claude/skills/evolve-config/SKILL.md {drift_seed} {drift_rate} <cited-findings-file>` to compute it — the script is the sole authority for how the **no-signal set** is derived and indexed (run it with no args for usage). Fitness-independent by construction: the candidate list is structural and only auditor-flagged traits are excluded, so the pick can never land on a trait that the historical-auditor's selection is acting on. **Empty no-signal set (every candidate was cited) → drift is a no-op this cycle** (the script exits 0 with no output in this case). <!-- CONFIG-ONLY -->Drift targets THIS SKILL's prose, never the config sources.
...
2. **Gather experience feedback (default-skip)** — Fires ONLY when the orchestrator prompt omits `experience_feedback` AND step 1 selected "Address operator-reported friction"; otherwise store `none volunteered` and move on — Phase 0's `bug-auditor` mines the permission-prompt/sandbox-denial signal deterministically, so the standard cycle needs no survey. When it fires, call `AskUserQuestion` (`multiSelect: true`, ≤4 options): `Permission prompts / sandbox friction`, `Model, effort or env-var settings`, `Hooks, statusline or UI behavior`, `Other (free-text follow-up)`. If `Other`, follow up free-text. Store as `{experience_feedback}`.
3. **Resolve today's date and the shared scratchpad** — Both values are single-homed in `evolve_preflight.sh` (DKT-292): capture `{today_date}` and `{scratchpad}` from the `today_date=`/`scratchpad=` lines emitted by the step-6 script run — do NOT hand-roll `date +%Y-%m-%d` or `echo "$TMPDIR/..."` here. `{today_date}` MUST be substituted into every spawning template so agents use a consistent date for changelog entries. `{scratchpad}` is emitted as the EXPANDED literal absolute path (`$TMPDIR/evolve-config-<today_date>`) — substitute THAT into every template, never an unexpanded `$TMPDIR/...` string (teammate `Read` takes absolute paths only, and the sandbox remaps `$TMPDIR` per context) — a SHARED, non-session-scoped path under the harness `$TMPDIR`, per the same convention as `.claude/skills/evolve-agents/SKILL.md` Pre-flight step 3 (a spawned teammate CAN Read an absolute path here; do NOT use a per-session scratchpad convention, which is session-isolated and unreachable by sibling teammates). After the step-6 run, `mkdir -p {scratchpad}/phase0` — the script does NOT create the scratchpad (only its own cache dir). Phase 0 completion writes the audit blocks there for Phase 1 to Read by path.
4. **Inventory config sources and the artifact name** — Run `wc -l src/user.rs src/user/claude_code.rs src/user/statusline.sh src/user/claude_code/hooks/teammate-idle-hook.sh 2>/dev/null`. Resolve the artifact name from the builder: `grep -n 'format!("{}-claude-code"' src/user.rs` confirms the config name suffix; the changelog artifact name is that suffix (`claude-code`). These sources have NO size budget (they are Rust/shell, not skill prose); the skill-population byte budget (evolve-skills pre-flight step 4 is authoritative) governs THIS SKILL.md only. Mode for SKILL.md is **TRIM** (over budget) or **BALANCED** (under budget) per its own `wc -c`. **Self-budget.** Like its evolve-* siblings this file sits well over the 10,000-byte skill rule and lives under `.claude/skills/`, outside the `byte_ceilings.tsv` `each` glob — outside the MECHANISM, not the CHARTER (`claude-5-paradigm-gate.md` §5 byte-ceiling scope caveat). Treat it as BALANCED with the changelog entry as its justification record; do NOT read the deferral above as putting this file in permanent TRIM against a target it cannot reach.
...
2. **Behavioral** — Does the setting change the deployed `settings.json` (or a script's behavior)? Reject: settings that serialize identically to the current output (no-op `with_*` calls). `src/user/claude_code/scripts/config_render_diff.sh 'cargo test --lib user::tests::prints_rendered_claude_code_config -- --nocapture'` mechanizes this by byte-diffing the rendered config at HEAD vs. worktree (exit 0=differs/PASS, 1=identical/FAIL), driving the live render target (`user::tests::prints_rendered_claude_code_config` in `src/user.rs`, DKT-94) — this is now the primary check; fall back to the serde-attribute read (Phase 1 step 3) only if cargo is unavailable.
...
2. `TaskCreate` all tasks up-front: Phase 0 ("Docs Research", "Config-History Audit", "Historical Audit", "Repetition Audit", "Bug Audit", "Innovation Scan", "Model Routing Audit"), "Review Config Genome", "Coherence", "Disambiguation", and "History Compaction".

| Phase | Agents | Lifecycle |
|---|---|---|
| 0 | `docs-researcher-phase0`, `config-history-auditor`, `historical-auditor`, `repetition-auditor`, `bug-auditor`, `innovation-scanner`, `model-routing-auditor` | Spawn parallel → all complete → shut down all before Phase 1 |
...
**Reviewer-tier note (prior-cycle finding P3 — intentional, not drift; governs BOTH reviewer seats, Phase 2 and Phase 3).** Phase 2 `coherence-reviewer` and Phase 3 `disambiguation-reviewer` spawn at `staff-engineer`/`opus` here, vs `distinguished-engineer`/`fable` (Phase 2 `coherence-reviewer`, unchanged) and `distinguished-engineer`/`opus` (Phase 3 `disambiguation-reviewer` — an operator-approved `Trial:` downgrade from `fable`, see evolve-agents'/evolve-skills' own changelogs) in evolve-agents/evolve-skills. This split is deliberate: auditing the BOUNDED Rust config genome (a finite `with_*`-setter / serde / script-path surface checked deterministically by `doctrine_check.sh`, `cargo check`, and path-match) is a narrower task-shape than open-ended agent/skill markdown-prose coherence, and `staff-engineer.md` lists `coherence-reviewer` among its own ephemeral spawns. Do NOT "fix" to gold for family symmetry.
...
Spawn SEVEN agents in parallel per the templates below: `docs-researcher-phase0` (staff-engineer), `config-history-auditor` (senior-engineer, needs Bash for read-only `git log` over the config sources), `historical-auditor` (senior-engineer, needs Bash for read-only grep/jq over `~/.claude/projects/`, `~/.claude/history.jsonl`, `.claude/agent-memory/`), `repetition-auditor` (senior-engineer, needs Bash, GLOBAL scope — the shared §4 template in `evolve-phase0-templates.md`, token-free), `bug-auditor` (senior-engineer, needs Bash, GLOBAL scope — mines permission-prompt and sandbox-denial events, a direct fitness signal for config surfaces 2 and 3; the shared §5 template, token-free), `innovation-scanner` (distinguished-engineer), and `model-routing-auditor` (senior-engineer, needs Bash). Skip `historical-auditor`, `repetition-auditor`, `bug-auditor`, and `model-routing-auditor` if pre-flight step 6 flagged SKIPPED. Assign Phase 0 tasks via `TaskUpdate`. **At Phase 0 completion the orchestrator Writes each agent's final `SendMessage` report verbatim to its own file** `{scratchpad}/phase0/<auditor>.md`, one per auditor name above — the Phase 1 reviewer Reads them by path; no Phase-0 report is ever inline-pasted into a prompt (a large token cut). **A SKIPPED (pre-flight step 6) or UNAVAILABLE (Crash & Stall Recovery) auditor still gets its file — write the literal sentinel string as the file's entire content** — so all 7 paths always exist and Phase 1 never special-cases a missing file.
...
3. **Verify the generated output.** A config change is not done until it compiles AND the build output is checked. Run `src/user/claude_code/scripts/config_verify.sh <edited-file>...` — it sequences all three checks in order and emits one PASS/FAIL/SKIPPED line each plus a single exit code: `cargo check` (catches a call to a non-existent setter or a type error the serde-attribute read cannot), `config_render_diff.sh` against the live render target (Content Gate check 2), and `bash -n` for any edited script. If cargo is unavailable both cargo steps report SKIPPED and the fallback is re-reading the `claude_code.rs` `#[serde(...)]` attribute for rename/skip semantics (a `skip_serializing_if` field set to its default produces NO output diff and fails the Content Gate Behavioral check). These checks prove the field SERIALIZES (static verification only); they do NOT prove runtime ENFORCEMENT of a permission/sandbox/hook rule, which is observable only after rebuild+redeploy in a fresh session — a runtime property measured by the next cycle's Phase 0, never claimed "verified" this cycle. **Phrase every acceptance criterion for a permission/sandbox change as a STATIC check** (grep the builder or generation site) and state runtime enforcement as out-of-scope with an explicit rebuild+redeploy+fresh-session route: an in-session runtime probe is not merely weak but MISLEADING — in a recorded attempt the non-excluded BASELINE command was denied too against a stale deployed `settings.json`, so neither a green nor a red result said anything about the artifact.
4. Writes/normalizes `docs/changelog/claude-code/config/<artifact-name>.md` per Changelog Format, appending the `Findings: <n> → <n> sub / <n> cos / <n> rej / <n> def` line to `### Summary` (counts from `findings_ledger_check.py`).
5. **Self-correct**: if a change worsens the config without behavioral gain, revert and retry.

**Defer parity-bound and CANONICAL-block findings to Phase 2 — never apply piecemeal.** Any Phase 1 finding that edits a `CANONICAL`-tagged block in THIS SKILL.md (the manifest-registered set — §Phase 2: Coherence step 3, never hardcoded) maintains byte-identical parity across the evolve-* family where shared; route to Phase 2 for a single family-wide lockstep call. Before adopting any newly-shipped settings field from the docs-researcher-phase0, read its official LIFECYCLE / clearing semantics, not just headline behavior, and confirm the `ClaudeCode` struct actually has (or needs) a setter for it — a field with no setter requires a `claude_code.rs` addition first. Check the prior changelog entry for an existing decision before re-proposing — a satisfied or rejected recommendation is a NO-OP, not a re-add. Before endorsing any lockstep propagation, verify every artifact the shared text references (script, ledger file, section, or template) exists in EVERY carrier — generic wording can be parity-safe while a referenced artifact is missing from a sibling, and propagating the paragraph there ships a phantom reference.
...
**evolve-config delta:** the reviewer Reads the freshly-coherent config sources (`src/user.rs`, `src/user/claude_code.rs`, the two scripts) and THIS SKILL.md; a CANONICAL-block or shared-frontmatter finding is applied family-wide in lockstep with byte-identity verification.
...
- `git log --oneline -30 -- src/user.rs src/user/claude_code.rs src/user/statusline.sh src/user/claude_code/hooks/teammate-idle-hook.sh`
...
- Findings must be Content-Gate-passing (Executable, Behavioral, Non-redundant, Concrete); catalogue nothing that already works.
...
Target: the Claude Code config genome — src/user/claude_code.rs (setters), src/user.rs (call chain), src/user/statusline.sh, src/user/claude_code/hooks/teammate-idle-hook.sh.
...
## Context
Date: {today_date} (for changelog). Read the latest changelog entry from docs/changelog/claude-code/config/<artifact-name>.md (may not exist — first cycle) and the full call chain in src/user.rs first.
...
## Task
1. Read the freshly-improved config sources: src/user.rs, src/user/claude_code.rs, src/user/statusline.sh, src/user/claude_code/hooks/teammate-idle-hook.sh — and THIS SKILL.md.
...
3. Verify every `CANONICAL:<TAG>` block THIS SKILL.md carries by running `doctrine_check.sh` per `.claude/skills/evolve-config/SKILL.md` §Phase 2: Coherence step 3 (Read it there — `doctrine_check_manifest.tsv` is the sole carrier authority; do NOT hand-enumerate). Flag any drift in a manifest-registered block.
...
## Task
1. Read the freshly-coherent, post-Phase-2 config sources: src/user.rs, src/user/claude_code.rs, src/user/statusline.sh, src/user/claude_code/hooks/teammate-idle-hook.sh — and THIS SKILL.md.
