...
Per ADR-0003 (`docs/tdd/adr/0003-centralized-vs-in-repo-pitfalls-memory-split.md`), recurring
pitfalls split by content into two homes per role: **in-repo**
...
`$HOME`-rooted, new) for lessons that generalize across repos. The block below carries the
write-time classification test.
