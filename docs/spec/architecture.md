...
scope: "System architecture: components, module boundaries, build/deploy data flow, and key design decisions of the Vorpal-based dotfiles manager and its Claude Code/Codex AI payloads."
...
The crate also ships **orthogonal AI-agent payloads**: a Claude Code agent-team configuration (`agents/claude-code/`, `skills/claude-code/`) symlinked into `~/.claude/`, and a Codex-native custom-agent/skill configuration (`agents/codex/`, `skills/codex/`) symlinked into `~/.codex/agents` and `$HOME/.agents/skills`. These concerns — environment provisioning and AI agent definition — share the same delivery mechanism (Vorpal artifacts) but are otherwise independent.
...
- Invokes the **config generators** (layer 3) to produce config-file artifacts for bat, Claude Code, Codex, Ghostty, K9s, OpenCode, a Neovim markdown ftplugin, a statusline script, and a TeammateIdle hook.
- Snapshots the `agents/claude-code/`, `skills/claude-code/`, `agents/codex/`, and `skills/codex/` source directories into artifacts via `FileSource`.
...
| `codex.rs` | `Codex` | TOML `config.toml` (model, features, agents, skills, sandbox, OTEL) | ~48 KB |
| `opencode.rs` | `Opencode` | JSON (`opencode.json`: keybinds, LSP, agents, themes) | ~79 KB |

The `claude_code.rs`, `codex.rs`, and `opencode.rs` generators are the largest and most security-relevant modules. `ClaudeCode` encodes the permission allow/ask/deny matrix, sandbox policy, and OTEL telemetry endpoints that govern the Claude Code team. `Codex` encodes model defaults, feature flags, custom-agent roles, skill paths, sandbox mode, shell environment policy, and OTEL exporters. These generators use `serde` with deterministic map ordering where applicable, which matters for content-addressed reproducibility.

### 4. Agent-team & skills payload (`agents/claude-code/`, `skills/claude-code/`, `agents/codex/`, `skills/codex/`)

Non-code payloads consumed by Claude Code and Codex at runtime:
...
- `agents/codex/*.toml` — seven Codex custom agents using the Codex schema (`name`, `description`, `developer_instructions`, optional nicknames).
- `skills/codex/*` — Codex-native ports of the project skills plus the `dev-team` workflow skill.
- `src/user/statusline.sh`, `src/user/teammate-idle-hook.sh` — shell scripts embedded into artifacts via `include_str!`.

This payload is delivered by being snapshotted (`FileSource`) and symlinked into `~/.claude/`, `~/.codex/agents`, and `$HOME/.agents/skills`. The Rust build still treats most contents as opaque directory snapshots, but `tests/codex_payload.rs` now parses the Codex agent TOML and skill frontmatter and verifies the Codex config registrations point at existing files.
...
graph TD
    subgraph repo["This repository"]
        ...
        PAYLOAD["agents/claude-code/ + skills/claude-code/<br/>agents/codex/ + skills/codex/<br/>shell scripts"]
    end
...
sequenceDiagram
    ...
    Bin->>SDK: build config generators (ClaudeCode, Codex, OpenCode, K9s, …)
    Bin->>SDK: FileSource(agents/claude-code/), FileSource(skills/claude-code/)
    Bin->>SDK: FileSource(agents/codex/), FileSource(skills/codex/)
    ...
    RT->>Home: create symlinks (settings.json, config.toml, configs, ~/.claude/*, ~/.codex/agents, ~/.agents/skills)
...
- **Payload boundary:** `agents/claude-code/`, `skills/claude-code/`, `agents/codex/`, and `skills/codex/` are data, not Rust code. They cross into the system via `FileSource` snapshots. Codex payloads have basic schema/frontmatter validation in tests; Claude markdown payloads remain runtime-validated only.
...
4. **AI payloads share the dotfiles delivery mechanism.** The Claude Code and Codex agent/skill definitions ride the same Vorpal artifact pipeline as the dotfiles. Benefit: single `vorpal build 'user'` provisions shell tooling plus both AI-agent surfaces. Cost: coupling — agent definitions are versioned and deployed in lockstep with dotfiles even though they are conceptually separate products.
...
- **No broad generator abstraction / limited tests on generators.** The config generators (`bat`, `ghostty`, `k9s`, `claude_code`, `codex`, `opencode`) emit security- and correctness-critical files. `src/user/codex.rs` has a serialization smoke test and `tests/codex_payload.rs` validates Codex payload shape, but the other generated configs remain largely untested. A malformed generated config could still ship if it is outside the covered Codex assertions. (Test posture is owned by `testing.md`.)
...
- **Partially typed agent/skill payload.** The Codex payload has automated validation for TOML shape, required skill frontmatter, role registrations, and Claude-only runtime syntax. The Claude markdown payload is still shipped verbatim with no schema validation in the Rust build, so Claude agent/skill errors are only caught at Claude Code runtime.
