export const meta = {
    ...
    whenToUse: 'Invoked by the docket-run skill after a wave returns and before its dispatch closes (the only back-fill window), and by the pause and resume paths for a wave whose usage was never back-filled. args is {dir: absolute transcript directory, mode: "steps" (default) | "seats", exclude: [STEP-N...] in steps mode or [seat...] in seats mode}. The script cannot read files itself; its agents run one fixed jq program per transcript and the reduction happens here.',
    phases: [
