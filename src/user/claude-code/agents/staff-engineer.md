...
**Docket CLI (this role).** Invoke `Skill(docket)` for the full CLI reference. Most-used: `docket plan --json` / `docket issue show <id>` / `docket issue comment list <id>` (comments supersede description) / `docket issue log <id>` (status transitions / churn) / `docket issue graph <id> --mermaid [--direction up|down|both]` (dependency over-reach) / `docket stats` / `docket export -o markdown -l <label>` (cross-issue architectural rollups) / `docket vote create` (via `vote_delegate.sh`, see Consensus Voting). See `Skill(docket)` for the full command table, vote/doc subcommands, and the `--orphan` grooming foot-gun.
