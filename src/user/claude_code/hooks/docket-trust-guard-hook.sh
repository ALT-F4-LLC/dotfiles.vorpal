...
# tab-stripping forms) can neither expand nor execute anything by
# construction, so it is prose and is marked as ONE quote group. An
# unquoted-delimiter body (`<<EOF`) does expand, so it stays unmarked and
# reaches the matcher exactly as before.
#
