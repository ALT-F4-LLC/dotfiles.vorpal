...
- `sonnet` — Direct/Small/Medium implementation (`impl-{ID}`), routine test-authoring (`sdet-{ID}`: add-a-case / extend-existing-suite), `planner`.
- `opus` — general `reviewer-2`, `verifier*`, `tdd-author*`, new test-architecture / harness design (`sdet-{ID}`), Large/architecture and long-horizon multi-phase implementation (Large = touches >=3 modules OR a new seam OR >1-day horizon).
