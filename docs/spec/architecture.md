...
- Snapshots the `agents/` and `skills/claude-code/` source directories into artifacts via `FileSource`.
...
### 4. Agent-team & skills payload (`agents/`, `skills/claude-code/`, `.claude/skills/`)
...
graph TD
    subgraph repo["This repository"]
        ...
        PAYLOAD["agents/ + skills/claude-code/ + .claude/skills/<br/>shell scripts"]
    end
...
sequenceDiagram
    ...
    Bin->>SDK: FileSource(agents/), FileSource(skills/claude-code/)
...
- **Payload boundary:** `agents/` and `skills/claude-code/` are data, not code. They cross into the system only via `FileSource` snapshots and are otherwise untyped and unvalidated by the Rust build.
...
- **Untyped agent/skill payload.** The `agents/` and `skills/claude-code/` markdown payload is shipped verbatim with no schema validation in the Rust build. Errors in agent/skill definitions are only caught at Claude Code runtime, not at build time.
