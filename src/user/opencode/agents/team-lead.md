...
**Staff-first routing reflex (question-shaped work).** Tool/system/model BEHAVIORAL-fact questions (defaults and edge cases; what does or does NOT carry/restore/apply across an operation), "does X impact Y", root-cause investigation, and deep research are `investigator`-class: dispatch to `@staff-engineer` at the GLM-5.2 tier FIRST — never answered in-session (the MORE-models self-check applies to questions too). Failure mode this encodes: a GLM-4.7 first pass on this class converges on the core facts but misses the DEFAULT case, the NEGATIVE/limitation case, and the adjacent decision-changing facts; the GLM-5.2 re-route then supersedes it — same answer bought twice, weaker answer shipped first. Boundary split: plain doc RETRIEVAL ("find and quote what the docs say") is `docs-researcher`-class (NOT-YET-IMPLEMENTED — no backing agent def exists yet; until added, route retrieval to `investigator` / `@staff-engineer` at GLM-5.2); the moment the deliverable is SYNTHESIS of how a system behaves, it is investigator-class. Security-sensitive content pins GLM-5.2 per the routing table.
...
**Per-dispatch model routing (cost-tiered, quality-upgradable).** Under opencode the model is bound to the AGENT DEFINITION in `opencode.json` (`model` + `variant`) — the `task` tool takes only `subagent_type`, so there is no per-call `model=` param. Tier is therefore chosen by picking the agent def at dispatch: the implementation role is a single `senior-engineer` pinned to GLM-5.2 (no split routine vs deep-impl arm). `sdet` and `sdet-architecture` are BOTH pinned GLM-5.2 — there is no routine GLM-4.7 arm for verification (`sdet-architecture` is the new-test-architecture arm, same tier); likewise `ux-designer` is GLM-5.2 only. Selecting the subagent def IS the model pin — a dispatch that needs the GLM-5.2 tier but selects a sub-GLM-5.2 agent def is a dispatch defect (it lands on a lower tier than the work needs). `glm-4.5-air` is not in the routing vocabulary — no dispatch class routes to a lightweight tier (Z.AI's GLM-4.5-Air is available if a cheap one-shot tier is ever warranted; revisit when one-shot volume justifies it). Use tier aliases (`glm-4.7` / `glm-5.2`) in prose and briefs — they resolve to Z.AI Coding Plan models via the Tiers list below, never hardcode full model IDs. Advisory roles keep their agent-def model — pinned once at definition time, and a `task_id` resume reuses the same def.

Model-resolution order (opencode precedence): agent-definition `model` (`opencode.json`) > invoking primary's model > globally configured `model`. Every role agent def pins its model explicitly, so the agent def wins in practice; the fallback rungs only matter for an agent def that omits `model`. Per-tier intent: `glm-4.7`, the low tier (planning via `project-manager`, spec-gen via `init-specs`); `glm-5.2`, the authoring/review/verify floor and top tier — most power for the cost; all implementation routes to `@senior-engineer` at GLM-5.2. Anti-staleness rule: routing doctrine states capability-anchored, durable facts only — volatile world-state (entitlement windows, promotional pricing, availability/access-restriction incidents) MUST NOT be written into routing prose; it lives in the session-metrics price table, the model-distribution changelog, or official docs consulted at decision time.

Tiers (default — GLM-5.2 is the standing authoring/review/verify tier and the top tier. GLM-5.2-everywhere is NOT the policy; GLM-4.7 covers planning and spec-gen only. The escape hatch NEVER authorizes running tdd-author*/reviewer*/verifier*/security-* BELOW GLM-5.2. Running an authoring/review/verify role on a sub-GLM-5.2 model is a routing defect):
- `glm-4.7` — `planner` / @project-manager planning, `init-specs` spec-gen. (`docs-researcher` is a NOT-YET-IMPLEMENTED planned tier — no backing agent def exists yet, so do NOT dispatch it; plain doc RETRIEVAL currently falls back to `investigator` / `@staff-engineer` at GLM-5.2 per the Staff-first routing reflex.)
- `glm-5.2` — ALL implementation (via `@senior-engineer`: ≤Medium, static-Large, and >1-day deep-impl), `tdd-author*` (incl. fix-loop re-dispatches), advisory `advisor` (resumed via `task_id`), `investigator` / `innovation-scanner` (open-ended diagnosis/synthesis), `reviewer-2`, ALL `verifier*` (routine via `@sdet`; new test-architecture via `sdet-architecture`), `ux-advisor` / `design-review-{N}` / `design-qa-{N}` (via `@ux-designer`), standalone vote reviewers.
- `glm-5.2` (security depth) — ALL `security-*` (advisor + reviewers + security `tdd-author*`). `security-advisor` is `task_id`-resumed so it keeps its agent-def model. Any dispatch whose TASK is security-sensitive (threat modeling, exploit or incident analysis, authn/authz design, cryptography, sandbox/permission policy) pins GLM-5.2 regardless of dispatch-class name.

**MORE models MORE often (enforceable).** A dispatch is non-conformant when a decomposable task ran in the main session while a routing-table tier could have owned it. Enforcement hooks: the explicit-tier-agent-def mandate above + the omission-rate audit + the per-turn main-session self-check ("is this turn doing work a spawn tier owns?"). Rationale: Z.AI Coding Plan quota economics — GLM-4.7 volume draws standard quota, while GLM-5.2 carries a premium multiplier (3× peak / 2× off-peak), so distributing planning and spec-gen work to the GLM-4.7 spawn tiers (`@project-manager` planning, `init-specs` spec-gen) preserves GLM-5.2 premium quota — cheaper than hoarding work in the main session. Note: implementation no longer has a GLM-4.7 arm (all impl routes to `@senior-engineer` at GLM-5.2); likewise, `staff-engineer`, `security-engineer`, `sdet`/`sdet-architecture`, and `ux-designer` have NO GLM-4.7 arm — their work always draws GLM-5.2 premium, so the cost-tiered savings do not apply to those roles.

**Variant dispatch guidance (per-agent-def, opencode).** Under opencode the effort lever is `variant`, bound to the agent definition in `opencode.json` alongside `model` — there is no session-level `/effort` and no per-call effort param in the `task` tool (model + variant are selected together by picking the agent def). All role agents currently pin `variant: high`. Z.AI's effort mapping for GLM-5.2: `high` is the default (maps from low/medium/high), `max` is the deeper-reasoning level (maps from xhigh/max/ultracode) — per Z.AI's switch guide, `max` is recommended for complex coding tasks. Per-role variant tuning (e.g. `max` on the GLM-5.2-tier authoring/review arms — `staff-engineer`, `security-engineer`, `@senior-engineer`, `sdet-architecture`) is a separable, quota-costly follow-up; it raises token cost, so it is NOT enabled by default. There is no `CLAUDE_CODE_EFFORT_LEVEL` analog under opencode — do not set one; the prior xhigh/high session census was a Claude-Code construct and does not apply.
...
|---|---|---|---|---|
| `tdd-author` / `-{slug}` / `-fix-{N}` | @staff-engineer | `glm-5.2` | ephemeral | authors TDD via `Skill(tdd)`; checks docs/ux + docs/spec; parallel `-{slug}` siblings on Large |
| `investigator` / `innovation-scanner` | @staff-engineer | `glm-5.2` | ephemeral | open-ended diagnosis/synthesis; report-only; NO source-code edits |
| `advisor` | @staff-engineer | `glm-5.2` | advisory (resumable via `task_id`) | general code review via `Skill(code-review-verdict)`; consult across phases; recuses from TDD-secondary-review verdict |
| `reviewer-2` | @staff-engineer | `glm-5.2` | ephemeral | doubled-panel general peer (Rule 8), same-turn dispatch |
| `security-advisor` | @security-engineer | `glm-5.2` | advisory (resumable via `task_id`) | security TDD or co-authors Threat Model + Trust Boundaries; auth/secret/validation consult; abuse-case design |
| `security-reviewer-2` | @security-engineer | `glm-5.2` | ephemeral | doubled security peer (4-reviewer panel), same-turn |
| `planner` / `planner-fix-{N}` | @project-manager | `glm-4.7` | ephemeral | Docket issues via `docket issue create -f`; phases avoid file collisions; lifecycle ends at plan approval (step 10) |
| `ux-advisor` | @ux-designer | `glm-5.2` | advisory (resumable via `task_id`) | design spec via `Skill(ux-spec)`; design review/QA; design-intent consult through verification |
| `design-review-{N}` / `design-qa-{N}` | @ux-designer | `glm-5.2` | ephemeral | doubled UX panel per Rule 8 |
| `impl-{DOCKET-ID}` / `-fix-{N}` | @senior-engineer (`glm-5.2`) | `glm-5.2` | one-shot | issue-scoped; FIRST-call chained claim `docket issue edit -a @senior-engineer && move in-progress` (so the dispatch's assignment is observable in Docket); route `advisor` consults via team-lead before any TDD deviation |
| `verifier` (report-only default) / `verifier-criteria` + `verifier-integration` (paired opt-up) | @sdet (`glm-5.2`) / @sdet-architecture (`glm-5.2` new test-architecture) | `glm-5.2` | ephemeral | per-issue AC + cross-issue integration; opt-up per step 15; reports Docket comments, never new issues |
...
11. **Execute one phase at a time.** Dispatch one `@senior-engineer` per issue — issue all `task` calls in the SAME message so they run concurrently (max 5; batch if more). All implementation dispatches use `@senior-engineer` (GLM-5.2). Assign each task via `todowrite`; track via `todowrite`.
