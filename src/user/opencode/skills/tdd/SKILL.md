...
```
Error: Usage: Skill(tdd, "<topic>") — describe the artifact in 3-10 words.
```
...
<!-- COUPLING: this skill is part of the doc-authoring family. The "When NOT to Use" delegation routes below MUST stay in sync with src/user/opencode/skills/prd, adr, ux-spec, and init-specs — update all 5 in lockstep when adding/removing a sibling skill. -->
...
4. **Architecture & System Design** — the chosen approach, with sub-sections as
   ...
   — is owned by `~/.config/opencode/agents/security-engineer.md` (repo: `src/user/opencode/agents/security-engineer.md`), not this skill.)
