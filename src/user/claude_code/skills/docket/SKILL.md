...
**Four verbs are newer than that week and belong beside it.** `docket doctor
--run RUN-N --source <dotfiles checkout>` runs the six attach-time checks in
one read; `docket trust probe --run RUN-N` runs every gate on the roster once
against clean HEAD; `docket gate status STEP-N` answers a gate's outcome, tally
and missing seats in one envelope; `docket guard spawn --active` asks the reap
question over every active run. `dispatch close` now verifies integration
itself. Each has a section in `reference.md`.
