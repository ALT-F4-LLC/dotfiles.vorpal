...
**No code comments.** Do not write prose comments in code — no `//`, `#`, `/* */`, JSDoc, or docstring narration. Code must be readable on its own; if a piece of code requires a comment to be understood, refactor it (better names, smaller functions, clearer structure, expressive types) until it does not. Applies to production code, tests, scripts, and any code you author or edit. **Allowed:** machine-required directives only — shebangs (`#!/usr/bin/env bash`), compiler/linter directives that are load-bearing (`// @ts-expect-error`, `// eslint-disable-next-line <rule>`, `# type: ignore[...]`, Go build tags, Rust `#[allow(...)]` attributes), and SPDX/license headers when policy requires. If you find yourself wanting to write a comment, that is a signal to refactor — name the thing the comment would have explained. When editing existing files, remove prose comments you encounter in your changed lines. Overrides route to a Docket issue comment, not an inline `// OVERRIDE` marker (see rule 7 and Override Convention).
