...
## 2026-06-12 — fix-loop ephemerals reopen already-closed Docket issues via the claim ritual, then can't re-close (auto-mode denies external-issue writes)
Symptom: impl-DKT-288-fix-2 ran the standard two-step claim (edit -a + move in-progress) on DKT-288, which was ALREADY closed from the original implementation round; its closing `docket issue close` was then denied by the auto-mode classifier (issue not session-created), leaving the issue stuck in-progress until team-lead re-closed it manually. Root cause: fix-round briefs reused the generic claim ritual wording, but a review-fix round operates on a CLOSED issue — the claim ritual reopens it and the ephemeral lacks permission to restore the state. Resolution: fix-loop briefs must say "do NOT claim or move the issue (it is closed); only `docket issue comment add`" — and team-lead's post-fix verification should include `docket issue show <id>` status, re-closing if a fix agent reopened it.

- **Edit anchor fails on skill-file escaped tokens** → evolve-* SKILL.md files escape `$ARGUMENTS` as `\$ARGUMENTS` on disk; an Edit old_string copied from prose (unescaped) misses → root cause: skill bodies escape shell-expandable tokens → resolution: copy anchor bytes from the Read output verbatim (incl. backslashes) before any SKILL.md edit (2026-06-12 evolve-config).
