...
**Invariants.**
1. Each skill's "driven by @X" / "callable ONLY by @X" / "typically @X" / "format authority" claim names an agent whose stated responsibilities own it: code-review → `@staff-engineer` (general 6-dim) + `@security-engineer` (security dim); tdd/adr → `@staff-engineer` (+ `@security-engineer` for security); verify-ac → `@sdet`; prd → `@project-manager`; ux-spec/design-review/design-qa → `@ux-designer`; simplify-scout → `@senior-engineer`; vote → any agent; init-specs → spawns `@staff-engineer`.
...
6. **`<!-- COUPLING -->` family-membership notes are self-consistent.** These notes declare which sibling skills must be edited in lockstep, so a stale note silently breaks the contract. Verified families: (a) **doc-authoring family of 5** — adr, prd, tdd, ux-spec each say "update all 5 in lockstep" and name the other four incl. init-specs (init-specs's own note is the reserved-name one, not "all 5"); (b) **report-emission family of 4** — code-review, verify-ac, design-qa, design-review each say "update all 4 in lockstep" naming the same four; (c) **init-specs↔prd reserved-name lockstep** — both say "update init-specs and prd in lockstep". **Invariant:** for each note, (i) stated member COUNT = actual carrier set size, (ii) each named sibling actually carries a matching note (except documented one-directional bridges), (iii) each member's "When NOT to Use" delegation list agrees with the family roster. **One-directional bridges (whitelisted, NOT drift):** simplify-scout's report-only analog note; ux-spec's bridge into the report-emission family.
7. **Work-doc references use the Docket-doc model, not file paths (post-DOC-4 migration).** Work documentation (TDD / ADR / PRD / UX) lives in Docket via `docket doc` under the `tdd|adr|prd|ux` taxonomy with `DOC-<n>` identity — discovered via `docket doc list -T <type>` and read via `docket doc show <DOC-id>`. **Invariant:** an agent or skill that references a work doc references it by the Docket-doc model (`docket doc list`/`show` + a `DOC-<n>` or `-T tdd|adr|prd|ux` discovery), NOT by a residual work-doc FILE PATH (`docs/tdd/`, `docs/tdd/adr/`, `docs/ux/`, or a PRD-style non-baseline `docs/spec/{slug}.md`). A surviving work-doc file path is rename/migration drift to FLAG — same class as a stale `verify`/`specs` skill name (#1). **Baseline `docs/spec/*` reads (selective spec reads — e.g. `architecture.md`, `performance.md`, `testing.md`, `security.md`, `code-quality.md`) and `docs/changelog/*` reads are NOT work-doc paths — they stay markdown by design; do NOT flag them.** Distinguish a baseline-spec filename (a fixed project-pattern file) from a PRD-style `docs/spec/{slug}.md` (a migrated work doc) by whether the path names a baseline spec convention or a per-feature slug.

**Detection (seeds).**
- Stale refs: `grep -rnE 'Skill\((verify|specs)\)|skills/(verify|specs)/' agents/*.md skills/*/SKILL.md .claude/skills/*/SKILL.md` → classify each `verify` hit bundled-runtime (legit) vs renamed-project-skill (drift) by context.
...
- Ladders: `grep -niE 'Blocker / Concern|Critical / High' skills/code-review/SKILL.md skills/design-review/SKILL.md` vs each agent citation; whitelist the design-qa "no Question" variant.
...
- Work-doc paths: `grep -rnE 'docs/tdd/|docs/tdd/adr/|docs/ux/' agents/*.md skills/*/SKILL.md .claude/skills/*/SKILL.md` → each hit is residual work-doc-path drift (flag). For `docs/spec/` hits, classify: a baseline-spec filename is legit (do NOT flag); a per-feature `docs/spec/{slug}.md` referenced as a PRD is migrated-work-doc drift. The baseline-spec set is the SEVEN init-specs-reserved names (authority: `skills/init-specs/SKILL.md`) — `architecture`, `security`, `operations`, `performance`, `code-quality`, `review-strategy`, `testing`; key the baseline-spec-legit classification on membership in that 7-name set, NOT a partial sample. Confirm the expectation holds in the reverse direction too — an agent/skill that DISCUSSES work docs should describe the `docket doc list -T <type>` / `docket doc show <DOC-id>` model, not a file-path model.
...
2. **CANONICAL:* block byte-parity** — each CANONICAL-tagged block is byte-identical across carriers **within its variant FAMILY** (modulo documented per-file exception). The "one global BANNER" model is FALSE — compare within the correct family. **The family rule keys on the OPENING-PREFIX STRING, never a hardcoded hash literal** (hashes drift the moment a banner is edited; no AC asserts a hash value). BANNER families:
   ...
   - (Agent-side CRITICAL banners are a separate per-role convention NOT wrapped in CANONICAL markers — check agent banners for *semantic* parity of the prohibitions, not byte-identity.)
   - `CANONICAL:PITFALLS` — all 7 agents, byte-identical (single-family). `CANONICAL:HARVEST` — evolve-agents + evolve-skills only, byte-identical (the marked blocks are identical; the agent-vs-skill distinction between the two cycles lives in surrounding prose OUTSIDE the markers). `CANONICAL:ARGUMENT_HANDLING` / `COLLISION_DIALOG` / `SAVE_AND_RETURN` — the 4 doc skills (adr, prd, tdd, ux-spec), byte-identical within the set.
...
4. **Doubling Rule / panel sizing** — `agents/team-lead.md` Rule 8 is source of truth (reference). Citing skills must not contradict its numbers: `skills/code-review/SKILL.md` single-default + opt-up-to-doubled language; `skills/vote/SKILL.md` panel-sizing table base 2/2/3/4, doubled 4/4/6/8 capped at 8.
...
```json
{
  "registry":           [{"dir": "code-review", "name": "code-review", "root": "skills|.claude/skills", "deployed": true}],
  "skill_refs":         [{"file": "agents/team-lead.md", "line": 251, "token": "code-review", "kind": "Skill-call|prose"}],
  ...
  "ladders":            [{"name": "staff-severity", "def_site": "skills/code-review/SKILL.md:350", "citations": ["agents/staff-engineer.md:170"]}],
  ...
  "workdoc_refs":       [{"file": "agents/staff-engineer.md", "line": 99, "path": "docs/tdd/", "kind": "workdoc-path-drift|baseline-spec-legit|changelog-legit"}],
...
`coupling_notes` (D3 #6), `workdoc_refs` (D3 #7 — residual work-doc file paths vs baseline-spec/changelog reads, classified by `kind`), `canonical_blocks.family`/`family_hash` (D4 #2), and `rule_presence.numbering_parse` (senior-engineer exception) are part of the pinned contract. `family_hash` is computed LIVE each run — pinned is the FIELD's presence, NEVER a fixed hash value.
...
## Task
Build the XREF index over ALL agents (agents/*.md) and ALL skills (skills/*/SKILL.md, .claude/skills/*/SKILL.md) using the detection seeds in the rubric (D1 registry/refs/frontmatter; D2 role_claims; D3 ladders/coupling_notes/workdoc_refs; D4 canonical_blocks/rule_presence). Compute family_hash LIVE per the D4 family rule (key on the opening-prefix string, strip trailing whitespace via `sed 's/[[:space:]]*$//'` before `shasum`). For workdoc_refs, classify each `docs/` hit by `kind` (work-doc path = drift; baseline-spec = legit by membership in the 7 init-specs-reserved names per D3 #7; changelog = legit). You do NOT judge — emit signals only.
...
## Task
Apply the D<n> invariants + detection seeds from the evolve-coherence rubric. HONOR the whitelist of intentional variants — do NOT mis-flag design-qa's "no Question" ladder, the byte-identical HARVEST blocks, the `Skill(name)` placeholder, the meta-instruction `Skill(verify-ac)` at staff-engineer.md:103, bundled-runtime `verify`, agent-banner semantic parity, the 3 BANNER families (incl. init-specs's shorter body + vote's singleton + leaf trailing clauses), or the one-directional COUPLING bridges (simplify-scout, ux-spec).
