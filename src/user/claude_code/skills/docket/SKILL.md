...
**v1 is frozen.** It will not gain fields. New response data appears under v2
only, which is what makes `--json` safe for existing scripts.
...
| `inputs` | [`"<step>.<kind>"` \| `"<step>.*"` \| `"issue.body"` \| `"issue.diff"`] | artifacts delivered to the step |
...
| `on_fail` | `"fix-loop"` \| `"waiting-human"` \| `"skip"` \| `"abandon-issue"`; default `"waiting-human"` | where a failure routes |
...
**A `type="human"` step must declare `on_fail` explicitly, and may not route
rejects to `waiting-human`.** The default is `waiting-human`, so a human gate
that declares nothing would park the issue on the resolution of the very thing
that just rejected it — a deadlock. Legal values there are `fix-loop`, `skip`,
and `abandon-issue`.
...
Gate results are recorded as `{gate, argv, exit, duration_ms, output,
truncated, verdict, reason}` with `verdict ∈ {pass, fail, unmatched}`. A
...
- Each held step is offered by `next --run` immediately, takes no claim and no
  token, and shows up as an ordinary human gate.
...
| Verb | Effect |
|---|---|
| `docket step approve <held>` | records a **new** artifact on the routing step with `operator_resolved: true` on **that cluster**, marks the held step `done` |
| `docket step reject <held>` | records **no** artifact for that cluster, marks the held step `done` |
...
Approval means *accept the computed value despite the spread*. Docket does not
read a disposition out of your note — the note is recorded verbatim for people
to read, and pulling a value out of prose would be docket interpreting what your
field means. The originally-held artifact stays addressable forever: what docket
computed and what you accepted are two records, not one overwritten one.
...
Nothing about the machinery below changes: voters cast with the same
...
Two positional args, `id` and target status.

| Flag | Short | Type | Default | Notes |
|---|---|---|---|---|
| `--if-version` | — | int | `0` | apply only at this version; `CONFLICT` otherwise (enforced even on a no-op move) |
...
| `--type` | `-T` | stringSlice | `nil` | repeatable |
| `--limit` | — | int | `10` | |
| `--run` | — | string | `""` | switches to STEP mode: lists a run's ready steps |

Watch-eligible.

**Two modes.** Without `--run` this is the issue-mode verb, unchanged. With
`--run RUN-N` it lists that run's ready STEPS instead, in the `next row` shape
below. The issue filters (`--status`, `--priority`, `--label`, `--type`) are
...
| `status` | effective status — `ready` here, computed, never stored |
...
| `--budget` | — | float64 | `0` | per-run cap, **enforced**; `0` means unlimited |

Creates a run in `planning`: nothing is bound, nothing is pinned, and no step
exists until `run activate`. A malformed or absent `--issue` refuses **before**
the run is created, so a typo leaves no empty run behind. A negative `--budget`
is `VALIDATION_ERROR` (exit 3); a missing `--request-file` is `NOT_FOUND`
(exit 2).
...
| `artifacts` | the **index**: id, kind, producer instance, sha256, bytes — never the bodies |
...
edited since activation does not reach a run already under way. Re-activating a
`done` or `abandoned` run is `CONFLICT` (exit 4).
...
path → `NOT_FOUND` (2); terminal run → `CONFLICT` (4).

#### `docket run pause|resume|abandon RUN-N` — `run_lifecycle.go`

| Flag | Short | Type | Default | Notes |
|---|---|---|---|---|
| `--reason` | — | string | `""` | required on `abandon` |
...
run whose cap moved has a trail saying so. `row_version` is bumped whether or not
...
| Verb | Writes | Effect |
|---|---|---|
| `dispatch open --run RUN-N` | yes | computes the ready set exactly as `next` does and records it |
...
| `--run` | string | `""` | filter to one run; omit for the repo-wide feed |
...
```
{ seq, at_ms, kind, run?, step?, step_id?, data }
```
...
Without `--run` the feed is repo-wide, which is the only place events belonging
...
| `human` | an operator verb, including one a harness relays | `run-*` (incl. `run-budget-set`), `step-claimed`, `step-heartbeat`, `step-recorded`, `step-resolved`, `step-approved`, `step-rejected`, `issue-abandoned`, `trust-*`, `dispatch-opened`, `dispatch-closed`, `reap-acknowledged`, `events-pruned` |

Three rows are worth their sentence. **`step-claimed` is `human`, not `next`**:
...
| `step approve\|reject STEP-N [--note …]` | no | `type="human"` gate steps **only**, declared or materialized |
...
the issue-level attempt trail, which is monotonic and never reset. Until vote
execution lands, `resolve` is also how an operator moves a run past a
`type="vote"` step.

#### `docket step approve|reject`

Both are **token-free**: a human gate is resolved by an operator who never
claimed it. Both apply to `type="human"` steps only — anything else is
`VALIDATION_ERROR` naming the step's actual class, because reaching for
`approve` usually means mistaking which step is blocking.

They also resolve a **materialized** `<step>-held` step, which is where an
`aggregate` step's held clusters are decided. The difference is where the
consequence lands: approving a declared gate finishes that gate, while approving
a held cluster un-defers the aggregate step's routing (see *Held clusters*
above). Deciding one twice is `CONFLICT` (exit 4) naming both steps.
...
| `guard gate --step NAME` | an **approved** `type="human"` step of that name exists for an active run |
...
**In a directory with no `.docket` up-tree, a guard ALLOWS (exit 0) rather than
...
`add` and `rm` write a `trust-added` / `trust-removed` event when run inside a
repository, carrying the argv **hash** rather than the argv — so a grant made
mid-run is auditable without leaking the command's arguments into a feed.
...
`VALIDATION_ERROR` (exit 3) at `set` time. See the Engine configuration section
above for the key table and defaults.
