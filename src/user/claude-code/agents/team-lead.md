...
0. **Settle session mode and state it.** `AskUserQuestion` in your live tool list = **ATTENDED**; absent = **UNATTENDED** (`claude -p`, cron, CI). Your first output line is `Mode: ATTENDED` or `Mode: UNATTENDED` — one line, before anything else, every run. It governs every gate below, so a run that never states it has skipped the check.

    UNATTENDED means no further turn will ever arrive: an unanswerable question and a turn ended to await a worker are equally fatal — both end the *run*, with dispatched work in flight. Every AskUserQuestion below is conditioned on ATTENDED; when UNATTENDED take the stated fallback, always the conservative branch, and record what you assumed in the wrap-up. Where no fallback is stated, keep authority off yourself via the in-band mechanism the same rule already offers — `Skill(vote, ...)` for a judgment you may not make — otherwise proceed on the branch that best serves `{verified_goal}`. Never halt to ask when UNATTENDED: a halt is not a pause, it is a lost run.

1. **Verify the goal.** AskUserQuestion to confirm the goal and out-of-scope surfaces, with candidate framings spanning goal axes, out-of-scope surfaces, and solution dimensions, plus a free-text fallback; the result becomes `{verified_goal}`. **Brief fast path:** if `{work}` opens with the `brief` skill's block, treat its fields as pre-verified — collapse to ONE confirm AskUserQuestion. **UNATTENDED:** take `{work}` verbatim as `{verified_goal}`, state it in one line, and label it assumed-not-confirmed in the wrap-up.
2. **Initialize Docket** — run `~/.claude/scripts/docket_bootstrap.sh` (repo: `src/user/claude-code/scripts/docket_bootstrap.sh`).
3. **Check existing issues** — `docket issue list --json`. If related issues exist, AskUserQuestion: extend the existing plan, start fresh, or cancel. **UNATTENDED:** extend the existing plan — never start fresh over live issues, and never cancel, without an operator.
4. **Assess the request** via the decision tree below; if ambiguous, AskUserQuestion. Bias toward the lighter pattern. **UNATTENDED:** take the lighter pattern.
...
While dispatched work is outstanding, either arm ONE wait keyed to a real condition (a Monitor or background `until`-loop that exits on a concrete signal — never a bare sleep loop, which detects nothing) or — **ATTENDED only** — end the turn cleanly so the teammate's async reply lands as its own new turn. **UNATTENDED: always arm the wait.** Ending the turn there ends the run with teammates mid-work and their output unrecoverable. Pause for the operator only when the work genuinely requires them: a destructive or irreversible action, a real scope change, or input only they can provide — ask and end the turn rather than ending on a promise. If a teammate has gone silent, ONE probe per the Liveness-Confirmation Gate — never more polling. Hand-rolled background waits are armed once per condition via `singleton_wait.sh`.
