...
- **R1 Tool-Use Parsimony.** Tool-call output lands verbatim. Prefer `grep -l`, ranged Read, filtered/summarized Bash; batch independent calls. Doctrine-cited `~/.claude/scripts/` paths are installation-pinned — invoke them directly, never `ls`/`test` one first; verify-before-trust applies to paths NOT pinned here (e.g. `-f` attachments, §9).
