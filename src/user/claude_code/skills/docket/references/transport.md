...
`--watch`/`-w` and `--interval` are hidden from `--help` on every command not
in this allowlist, defined in `internal/cli/watch_commands.go`. The flags are
shared persistent globals, so they exist on every command even where help
omits them.
...
`--watch` on any other command — read-only (`docket project list`) or not —
fails with a `VALIDATION_ERROR` whose message enumerates the live allowlist:
`--watch is limited to: docket board, docket config, ...` (generated by
`watchRejectionMessage()`). That message is authoritative if this copy ever
drifts.

### Free-text flags: quote so the shell cannot run your text

Free-text flags — `issue create`/`issue edit` `-d`, `issue comment add -m`,
`doc create -d`, `vote cast --summary`, `step fail`/`step approve`/`step
reject` `--note` — often carry pasted content: a bug report, another agent's
output, a transcript excerpt, command names in backticks. Inside a
double-quoted argument the shell runs backtick and `$(...)` spans, with your
cwd, permissions, and network, before docket sees the argv.

Safe forms, depending on the flag's supported input modes:

- a supported file flag, such as `vote cast --summary-file`, or a supported
  `@path` value;
- a correctly single-quoted literal, escaping embedded apostrophes;
- stdin, when documented, via a quoted heredoc delimiter: `docket … -d -
  <<'DESC'` … `DESC` (pick a delimiter the body cannot contain: a body
  quoting a shell snippet with its own bare `EOF` line closes an `EOF`
  heredoc early and runs the remainder as commands).

A double-quoted `echo "…" | docket … -d -` still expands before the pipe, and
an unquoted heredoc (`<<EOF`) expands too. Prefer file/stdin transport for
untrusted or multiline bodies; do not assume every free-text flag implements
the `-` stdin sentinel.

### JSON envelope shape

JSON envelope responses are single-line objects on stdout. Stream, source/body,
and export modes have command-specific contracts; check help before treating
raw output as an envelope.
...
V1 preserves legacy response shapes with the additive amendments below. Use v2
for new agent integrations, for uniform collections and version fields. Do not
depend on an exact serialized key set or byte sequence across releases.
...
Exit code `0` is command success, not proof the command's subject passed a
gate. Ordinary store-dependent commands return `NOT_FOUND` when the store is
absent; initialize it only when creating a store is within the request.
Guards instead treat an absent store as not applicable.
...
Omitting `--if-version` preserves last-writer-wins behavior. The version still
increments, so a concurrent CAS writer detects the change.

Read the current version from `.data.version` under `--json=v2`; v1 payloads
...
Keys are scoped per verb: the same key on `issue create` and `doc create` is
...
`docket issue claim` takes a lease on an issue and mints a capability token.
The claim is atomic: exactly one of any number of concurrent claimants wins,
and the losers get `CONFLICT` (exit 4).
...
heartbeat DKT-1 --json=v2`) or set the environment on that invocation. Do not
print capabilities into logs. Shell variables need not survive separate Bash
calls: use protected runner state or an owner-only temporary file outside the
tracked tree, and clean it up when the claim ends.
...
`docket issue close` ends a live lease as a side effect. The holder may
always close; a non-holder closing a live-leased issue is refused
`AUTH_ERROR`. Closing an unclaimed issue needs no token.
...
can be re-run on an interval instead of polling manually:
