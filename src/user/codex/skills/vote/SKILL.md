...
> **CRITICAL — applies to coordinator AND every spawned reviewer:** (1) Do NOT commit ANY changes (no `git add`, `git commit`, or `git push`) unless EXPLICITLY instructed by the user. (2) Reviewers MUST NOT spawn sub-agents, invoke `/vote` recursively, invoke other skills recursively, or form/manage a team — they are independent leaf reviewers per the protocol. (3) **Team-mode callers MUST NOT invoke `(vote)` directly** — delegate via send_input to team-lead per the Delegation Protocol below; direct invocation spawns a nested team and is rejected. Standalone `/vote` invocation by the operator is the only direct entry.
...
If you were spawned as a teammate (an agent inside an existing team with a lead to send_input, not invoked standalone via `/vote`), you MUST NOT spawn agents or form teams — use the **Delegation Protocol** below. Otherwise (standalone via `/vote`), execute the full protocol from Pre-flight.
...
3. **Delegate** — `send_input(to="team-lead", message={type: "delegation_request", protocol_version: "1", skill: "vote", request_id: "{uuid}", vote_id: "{vote-id}", from: "{your-agent-name}", summary: "{one-line}", artifact?: "{path}"})`. `summary` and optional `artifact` are operator-observability hints only — the authoritative proposal lives in docket. Wait for `delegation_response` with matching `request_id`.
4. **Expected response shape** — `{type: "delegation_response", request_id: "{uuid}", status: "completed|failed|escalated", vote_id: "{vote-id}", reason?: "{string}"}`. team-lead invokes `(vote, "{vote-id}")` standalone (the vote_id branch skips Phase 1) and forwards the result — see team-lead.md Consensus Integration for the relay contract.
...
Select reviewers based on domain relevance to the proposal. Each reviewer spawn starts a fresh instance — never reuse a long-lived teammate for consensus.
...
Spawn reviewer agents **in parallel** using the Reviewer Prompt Template below — the template encodes the full reviewer contract (proposal, rationale, checklist, structured output, delivery, isolation from other reviewers). Set each reviewer's task to `in_progress` immediately after spawning (`local ledger update(taskId=<id>, status="in_progress")`) and to `completed` after its review arrives. Reviewers are teammates: their plain final-turn text is NOT visible to you — each review arrives ONLY via send_input per the template's Delivery section. Parse verdict, confidence, domain_relevance, and findings from each send_input payload before proceeding to Phase 3; a reviewer that goes idle without delivering is a failed reviewer (below).
...
## Delivery (MANDATORY)
send_input the COMPLETE structured review above to team-lead (your coordinator) — your plain final-turn text is NOT visible to the coordinator, so an un-sent review is a failed review. Then stop; the coordinator closes the returned worker id after consuming the review.
...
4. If invoked by another agent, use **send_input** to deliver the consensus result
...
3. Notify the caller with `[VOTE] Consensus not reached (score: {score}, threshold: {threshold})` plus the aggregated findings. If invoked by an agent, send via send_input with the three options inline. If invoked by the user, use AskUserQuestion with `header: "Next step"`, options: `[{label: "Revise and re-vote", description: "Address findings and run a new round"}, {label: "Escalate", description: "Hand off to human decision"}, {label: "Abort", description: "Stop here, no further rounds"}]`.
