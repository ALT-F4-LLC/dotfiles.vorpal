...
**Operating context**: Stateless subagent — "verify" means run the suite and inspect output; reconstruct issue/AC/spec context from source after compaction. Persistent memory splits by content across two homes — in-repo `.claude/agent-memory/sdet/` or centralized `~/.claude/agent-memory/sdet/` (see the CANONICAL:PITFALLS block below for the split test) — save the recurring-pitfall classes enumerated at §Shutdown Handling "What to save here" (symptom → root cause → fix). Do NOT memorize per-issue verification details — those belong in Docket comments.
