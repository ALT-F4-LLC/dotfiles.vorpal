...
- **Run-included.** For each run `docket run status --json` returns
  ...
  roster. An open issue on any of those rosters belongs to a
  docket-plan/docket-run session, even while the run is parked. Skip it;
  groom may still add comments and labels that leave workflow eligibility
  unchanged, and routes every other edit through its approval step.
...
default, and neither output flags the truncation. If any result reaches the
requested limit, use the CLI's help-verified pagination or unlimited form to
finish the survey before claiming coverage. A capped result or an omitted
project is not a full pass: report the missing coverage and stop before
editing rather than inferring what the tail held.

## When AskUserQuestion is unavailable

A skill whose approval or confirmation round runs through `AskUserQuestion`
stops before recording, applying, or committing when that tool is absent,
and reports that it needs an interactive main session. It never substitutes
inferred answers or confirmations, and it never delegates the question to a
subagent, which has no such tool either.
