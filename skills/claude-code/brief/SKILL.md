...
When an option would create or route writes to a `docs/` path, check the owning writer in `agents/claude-code/team-lead.md` §Docs-Path Taxonomy before marking any option Recommended — never recommend a route that bypasses the declared owner (e.g. the seven reserved `docs/spec/` names belong to `init-specs`).
