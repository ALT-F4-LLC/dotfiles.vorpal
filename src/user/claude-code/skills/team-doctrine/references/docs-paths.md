...
**Skill-file locations (adjacent "where does it live" rule, NOT a `docs/` path).** `evolve-*` skills live ONLY under `.claude/skills/<name>/SKILL.md` (project-scoped; NEVER deployed to `~/.claude/`). Every OTHER skill lives ONLY under `~/.claude/skills/<name>/SKILL.md` (deployed) or `src/user/claude-code/skills/<name>/SKILL.md` (repo source) — this repo's `.claude/skills/` holds ONLY the 5 evolve-* dirs. Reading `.claude/skills/vote/SKILL.md` or `~/.claude/skills/evolve-skills/SKILL.md` is the recurring wrong-tree miss. **Repo-root vs source-tree:** `docs/{spec,tdd,adr,ux,changelog}/` and the top-level `.claude/` tree (agent-memory, skills, settings) live ONLY at the repo root — NEVER under `src/user/claude-code/`, which is the SOURCE tree for deployed agents/skills/scripts only (`src/user/claude-code/{agents,skills,scripts}/`). Writing `src/user/claude-code/docs/…` or `src/user/claude-code/.claude/…` is the recurring dual-path miss. **Citing a source definition (prefix discipline).** When you CITE this repo's own agent/skill/script SOURCE by repo-relative path, that path ALWAYS starts with `src/user/claude-code/` (e.g. `src/user/claude-code/agents/staff-engineer.md`, `src/user/claude-code/scripts/slug.sh`) — a bare `agents/…`/`skills/…`/`scripts/…` citation, or a deployed `.claude/…`/`~/.claude/…` prefix substituted for a SOURCE definition, is the recurring wrong-prefix miss (3 sessions). Sole exception: the 5 evolve-* skills, which exist ONLY at `.claude/skills/<name>/SKILL.md` with no `src/` counterpart — cite them there.
