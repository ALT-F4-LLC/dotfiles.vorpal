...
get_git_info() {
    ...
    if [ -f "$cache_file" ]; then
        ...
        now=$(date +%s)
        # GNU stat first: on coreutils, -f means --file-system and dumps a
        # multi-line report to stdout before failing, so it cannot be the
        # first attempt. BSD stat rejects -c with no stdout, so it falls through.
