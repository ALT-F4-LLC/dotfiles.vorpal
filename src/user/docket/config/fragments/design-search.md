...
candidates and choose on the merits. The first answer that fits is a candidate,
not a decision. The delivered change is still the smallest clear implementation
of the winner: code-philosophy and the laziness ladder govern what you ship,
this fragment governs what you weigh before shipping.
...
- **Derive from the invariant, not the template.** Start from the actual
  contract: the requirement, the callers, the data, the failure. The answer
  that would look the same in any codebase in this language is a candidate to
  beat, not the default. Ask what this problem needs that the familiar shape
  does not provide, and what the familiar shape carries that this problem
  does not need.
- **Weigh at least one candidate that is not the existing path.** Extending
  ...
  code-philosophy #9 and rung 2 of the laziness ladder; it is not the only
...
- **Record the search.** The emitted artifact names the candidates weighed,
  the one chosen, and why it won, in a few lines. Ending at the obvious
  answer is a fine result when the record shows the alternatives lost on the
