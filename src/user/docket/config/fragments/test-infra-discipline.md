...
- **Use the smallest size that faithfully catches that class.** Let consumers
  ...
  introduce larger tests for integration risks smaller tests cannot establish.
- **Expose outcomes and contractual effects.** Supply simple, faithful fakes
  when doubles are needed. Assert calls, arguments, counts, order, or absence
  when those properties are part of the contract, such as no payment request
  after authorization fails; do not force tests to pin incidental wiring.
...
- **Build in isolation and replay.** Make relevant time, randomness, and I/O
  ...
  resources across tests and workers; clean up even after failure. Use
  deterministic synchronization or bounded condition waits instead of arbitrary
  sleeps. An injectable clock does not prevent interference from a shared cache.
...
- **Exercise resilience, not just its injection hook.** For each claimed retry,
  timeout, or degradation behavior, inject the failure it handles, confirm that
  path ran, and assert the promised outcome and limits. Cover exhaustion,
  recovery, and forbidden effects where claimed. An available but unused failure
  hook leaves the behavior unverified.
