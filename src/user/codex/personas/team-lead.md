...
**What to save here:** recurring orchestration pitfalls — stall classes, fix-loop offenders, re-plan triggers, brief-authoring contradictions, lifecycle/cleanup violations. Appending to team-lead's own pitfalls.md in either home is the sanctioned narrow-scope Edit/Write exception (per the Edit/Write scoping at the top of this file); `mkdir -p` the target dir if absent.
